/* ══════════════════════════════════════════════════════════════════════
   RADIO TRIGAL FM — IDIOMA DE LOS MENÚS (i18n.js)
   ══════════════════════════════════════════════════════════════════════
   Único fichero que sabe **en qué idioma está escrito cada texto de la
   interfaz** y cómo pasar de uno a otro. Solo cubre los MENÚS y la
   estructura de la interfaz (pantallas de navegación, botones, ajustes,
   cabeceras, overlays de confirmación/resultado...) y los nombres de
   región que se le muestran al jugador, la Guía de Juego
   (`#screen-guide` en index.html, claves "guide.*") y los títulos de
   canciones (parcialmente — ver la sección "song.*" de cada bloque de
   idioma, con el nombre oficial del lugar cuando existe traducción). A
   propósito NO traduce el resto de contenido de datos del juego:
   descripciones de logros y Eventos Pokémon (game.js/pokemon.js) —
   eso sigue solo en español (salvo el catálogo de Logros, "achv.*",
   que si tiene traducción).

   Este fichero no decide nada de reglas de juego ni pinta directamente
   nada complejo: solo expone `t(key, vars)` (traduce una clave),
   `tData(key, defaultText, vars)` (traduce, si existe traducción, un
   texto que vive como valor por defecto en otro fichero) y
   `regionDisplayName(region)` (traduce, si existe traducción, el
   nombre de una región — p. ej. "Teselia" → "Unova" en inglés), más
   `applyTranslations()` (aplica el idioma actual a todo el marcado
   estático de index.html) y `setLanguage(lang)` para cambiarlo. El
   resto de ficheros (ui.js/game.js) llaman a estas funciones cuando
   generan a mano un texto que antes estaba en español "a pelo", en vez
   de duplicar las cadenas. Importante: la clave interna de cada región
   (`REGIONS`/`REGION_META` en game.js, el campo `region` de cada
   canción, `localStorage`...) NUNCA pasa por `regionDisplayName()` —
   solo el texto que ve el jugador.

   Se carga justo después de `storage.js` (necesita leer/guardar
   `settings.language`) y antes de `audio.js`/`pokemon.js`/`ui.js`/
   `game.js`, que son quienes llaman a `t()`.
   ══════════════════════════════════════════════════════════════════════ */

// ═══════════════════════════════════════════════
//  🌐 DICCIONARIO DE TEXTOS
// ═══════════════════════════════════════════════
// Cada clave usada en index.html (atributo data-i18n="...") o en una
// llamada a t("...") desde ui.js/game.js debe existir en AMBOS idiomas.
const I18N = {
  es: {
    "splash.tap": "Toca la pantalla para empezar",
    "splash.subtitle": "🎵 PokéQuiz · Music Edition",
    "brand.radioTrigal": "Radio Trigal",
    "brand.fm": "FM",

    "common.close": "Cerrar",
    "common.trainerDefault": "Entrenador",

    "pokeEventsInfo.btnAria": "Información de los Eventos Pokémon",
    "pokeEventsInfo.title": "🎲 Eventos Pokémon",
    "pokeEventsInfo.subtitle": "Al azar, durante una ronda puede aparecer uno de estos Pokémon con un efecto especial:",

    "profileSetup.title": "¡Bienvenido, entrenador!",
    "profileSetup.desc": "Antes de empezar, elige tu nombre de entrenador y tu imagen de perfil.",
    "profileSetup.namePlaceholder": "Tu nombre de entrenador",
    "profileSetup.avatarLabel": "Elige tu imagen de perfil",
    "profileSetup.confirm": "✔ ¡Empezar a jugar!",
    "profileSetup.checking": "Comprobando…",
    "profileSetup.nameTaken": "Ese nombre de entrenador ya está en uso. Elige otro.",

    "profile.editNameTitle": "Cambiar nombre",
    "profile.changeAvatarBtn": "🖼️ Cambiar imagen de perfil",
    "profile.levelLabel": "Nivel",
    "profile.xpLabel": "{into} / {need} XP para el nivel {next}",

    "profile.stats.title": "Estadísticas",
    "profile.stats.totalPoints": "💰 Puntos totales",
    "profile.stats.achievementsUnlocked": "🏅 Logros desbloqueados",
    "profile.stats.gamesPlayed": "🎮 Partidas jugadas",
    "profile.stats.correctAnswers": "✅ Respuestas correctas",
    "profile.stats.bestStreak": "🔥 Mejor racha",
    "profile.stats.infiniteRecord": "♾️ Récord Desafío Infinito",
    "profile.stats.storyRecord": "📖 Récord Modo Historia",
    "profile.stats.hardRecord": "🔴 Récord Modo Difícil",
    "profile.stats.combatRecord": "⚔️ Récord Modo Combate",
    "profile.stats.regionRecordsTitle": "🗺️ Récords por región",
    "profile.stats.sonidexUnlocked": "🎼 Fichas de la Sonidex desbloqueadas",
    "common.pts": "pts",

    "lock.badge": "Bloqueado",
    "lock.levelReqBadge": "Se desbloquea al alcanzar el nivel {level} de perfil",
    "lock.achievementReqBadge": "Desbloquea con el logro «{title}»",
    "lock.levelReqToast": "Alcanza el nivel {level} de perfil para desbloquear",
    "lock.achievementReqToast": "Consigue «{title}» para desbloquear",
    "avatar.lockedTitle": "{name} (bloqueado — requiere {req})",
    "avatar.lockedToast": "Consigue {req} para desbloquear este avatar",
    "avatar.lockReqLevel": "nivel {level} de perfil",
    "avatar.lockReqAchievement": "el logro «{title}»",
    "avatar.lockReqUnknown": "un logro",

    "streaks.easyMode": "Modo fácil",
    "streaks.hardMode": "Modo difícil",
    "streaks.combatMode": "Modo combate",
    "streaks.infiniteMode": "Desafío infinito",
    "streaks.normalByRegion": "Modo normal · por región",
    "streaks.easyLabel": "🟢 Fácil",
    "streaks.hardLabel": "🔴 Difícil",
    "streaks.combatLabel": "⚔️ Combate",
    "streaks.infiniteLabel": "♾️ Desafío",

    "ach.starBadgeTooltip": "Desbloquea una función especial",
    "ach.unlockedOn": "Desbloqueado el {date}",

    "story.regionCompleted": "{region} Completado",

    "header.back": "Atrás",
    "header.points": "Puntos",
    "header.round": "Ronda",
    "header.streak": "Racha",

    "home.tagline": "Tu emisora Pokémon",
    "home.play.title": "Jugar",
    "home.play.desc": "5 modos y Minijuegos",
    "home.story.title": "Modo Historia",
    "home.story.desc": "Recorre todas las regiones, de Kanto a Alola",
    "home.achievements.title": "Logros",
    "home.sonidex.title": "Sonidex",
    "home.leaderboard.title": "Clasificaciones",
    "home.options.title": "Opciones",
    "home.achSummaryDefault": "Consulta tus logros desbloqueados",
    "home.sonidexSummaryDefault": "Desbloquea canciones acertándolas 10 veces",

    "modes.easy.title": "Modo Fácil",
    "modes.easy.desc": "Solo adivina la región (Kanto, Johto, …)",
    "modes.normal.title": "Modo Normal",
    "modes.normal.desc": "Primero elige región, luego adivina la canción",
    "modes.hard.title": "Modo Difícil",
    "modes.hard.desc": "6 opciones · canciones de cualquier juego · 10s por ronda",
    "modes.combat.title": "Modo Combate",
    "modes.combat.desc": "6 opciones · Adivina las canciones de combate",
    "modes.infinite.title": "Modo Desafío Infinito",
    "modes.infinite.desc": "6 opciones · Rondas sin límite · un fallo termina la partida",
    "modes.other.title": "Minijuegos",
    "modes.other.desc": "Laboratorios · Bicicleta · Otras Sagas",
    "modes.endlessToggle.title": "Modo infinito: rondas sin límite, un fallo termina la partida",

    "region.title": "Modo Normal",
    "region.subtitle": "Elige una región. Las canciones y respuestas serán solo de esa región.",

    "other.title": "Minijuegos",
    "other.subtitle": "Selecciona una categoría. Solo sonarán canciones de esa opción.",
    "other.centro.title": "Centro Pokémon",
    "other.centro.desc": "Solo canciones y opciones de Centro Pokémon",
    "other.lab.title": "Laboratorios",
    "other.lab.desc": "Solo canciones y opciones de Laboratorios",
    "other.bike.title": "Bicicletas",
    "other.bike.desc": "Solo canciones y opciones de Bicicletas",
    "other.surf.title": "Surf",
    "other.surf.desc": "Solo canciones y opciones de Surf",
    "other.titlescreens.title": "Pantallas de Título",
    "other.titlescreens.desc": "Solo canciones y opciones de Pantallas de Título",
    "other.callevictoria.title": "Calle Victoria",
    "other.callevictoria.desc": "Solo canciones y opciones de Calle Victoria",
    "other.openinganime.title": "Openings del Anime",
    "other.openinganime.desc": "Solo canciones y opciones de Openings del Anime",
    "other.mysterydungeon.title": "Pokémon Mundo Misterioso",
    "other.mysterydungeon.desc": "Solo canciones y opciones de Mundo Misterioso",
    "other.colosseum.title": "Pokémon Colosseum / XD",
    "other.colosseum.desc": "Solo canciones y opciones de Colosseum/XD",
    "other.ranger.title": "Pokémon Ranger",
    "other.ranger.desc": "Solo canciones y opciones de Ranger",

    // Openings del Anime: pantalla previa de selección de doblaje (solo se
    // muestra jugando en español — ver el listener de [data-other] en game.js).
    "openingsLang.title": "Openings del Anime",
    "openingsLang.subtitle": "¿Con qué doblaje quieres las canciones?",
    "openingsLang.spain.title": "Español de España",
    "openingsLang.spain.desc": "Los openings tal y como se emitieron en España",
    "openingsLang.latino.title": "Español Latino",
    "openingsLang.latino.desc": "Los openings con el doblaje latinoamericano",

    "options.title": "Opciones",
    "options.subtitle": "Ajusta el audio y la apariencia.",
    "options.music.label": "Música",
    "options.music.desc": "Volumen de la canción",
    "options.sfx.label": "Efectos de sonido",
    "options.sfx.desc": "Volumen de acierto/error",
    "options.dark.label": "Modo Oscuro",
    "options.dark.desc": "Interfaz más oscura para poca luz",
    "options.toggleOn": "Activado",
    "options.toggleOff": "Desactivado",
    "options.saveNote": "💾 Estas opciones se guardan automáticamente.",
    "options.graphics.title": "🖼️ Opciones gráficas",
    "options.bg.label": "Fondo animado",
    "options.bg.desc": "Cielo, nubes y Pokémon caminando de fondo",
    "options.particles.label": "Efectos de partículas",
    "options.particles.desc": "Estrellas y chispas al acertar o desbloquear logros",
    "options.cursor.label": "🖱️ Puntero del ratón",
    "options.cursor.desc": "Sustitúyelo por un sprite desbloqueado con logros",
    "options.cursor.normalName": "Normal",
    "cursorStyle.poke_flute.name": "Poké Flauta",
    "cursorStyle.poke_ball.name": "Poké Ball",
    "cursorStyle.super_rod.name": "Supercaña",
    "cursorStyle.rare_candy.name": "Caramelo Raro",
    "cursorStyle.shuckle.name": "Shuckle",
    "cursorStyle.cosmog.name": "Cosmog",
    "cursorStyle.togedemaru.name": "Togedemaru",
    "cursorStyle.zygarde.name": "Zygarde",
    "cursorStyle.dedenne.name": "Dedenne",
    "cursorStyle.emolga.name": "Emolga",
    "cursorStyle.rotom.name": "Rotom",
    "cursorStyle.pachirisu.name": "Pachirisu",
    "cursorStyle.plusle.name": "Plusle",
    "cursorStyle.pichu.name": "Pichu",
    "cursorStyle.mew.name": "Mew",
    "cursorStyle.bulbasaur.name": "Bulbasaur",
    "cursorStyle.squirtle.name": "Squirtle",
    "cursorStyle.charmander.name": "Charmander",
    "cursorStyle.meowth.name": "Meowth",
    "cursorStyle.charizard_y.name": "Charizard Y",
    "cursorStyle.charizard_x.name": "Charizard X",
    "cursorStyle.pikachu.name": "Pikachu",
    "cursorStyle.spinda.name": "Spinda",
    "cursorStyle.ludicolo.name": "Ludicolo",
    "cursorStyle.manaphy.name": "Manaphy",
    "cursorStyle.magnemite.name": "Magnemite",
    "cursorStyle.chansey.name": "Chansey",
    "cursorStyle.rayquaza.name": "Rayquaza",
    "cursorStyle.jigglypuff.name": "Jigglypuff",
    "cursorStyle.regigigas.name": "Regigigas",
    "cursorStyle.rayquaza_shiny.name": "Rayquaza shiny",
    "cursorStyle.meloetta.name": "Meloetta",
    "cursorStyle.lockedTitle": "{name} (bloqueado — requiere {req})",
    "cursorStyle.lockedToast": "Consigue {req} para desbloquear este puntero",
    "options.language.title": "🌐 Idioma",
    "options.language.desc": "Idioma de los menús, logros, Eventos Pokémon y Guía de Juego",
    "options.language.es": "Español",
    "options.language.en": "English",
    "options.language.ja": "日本語",
    "options.guide.label": "📘 Guía de Juego",
    "options.guide.desc": "Modos, puntuación, logros y Sonidex explicados",

    "guide.header.title": "📘 Guía de Juego",
    "guide.header.subtitle": "Todo lo que necesitas saber para dominar Radio Trigal FM.",
    "guide.modes.title": "🎮 Modos de juego",
    "guide.modes.easy.title": "🟢 Modo Fácil",
    "guide.modes.easy.desc": "Suena una canción y solo tienes que acertar de qué <b>región</b> es (Kanto, Johto, Hoenn…). Es el modo más sencillo: ideal para empezar. Las partidas duran 10 rondas; fallar no te elimina, solo corta tu racha, así que siempre juegas las 10 rondas completas.",
    "guide.modes.normal.title": "🔵 Modo Normal",
    "guide.modes.normal.desc": "Primero eliges una región y después tienes que adivinar el <b>título exacto</b> de la canción entre varias opciones de esa misma región. 10 rondas; como en Fácil, fallar no termina la partida, solo corta tu racha.",
    "guide.modes.hard.title": "🔴 Modo Difícil",
    "guide.modes.hard.desc": "Las canciones pueden ser de <b>cualquier región</b>, se eligen entre 6 opciones y solo dispones de 10 segundos por ronda para responder. 10 rondas, sin eliminación por fallo. Se desbloquea al alcanzar el <b>nivel 5 de perfil</b>.",
    "guide.modes.battle.title": "⚔️ Modo Combate",
    "guide.modes.battle.desc": "Igual que el Difícil pero solo con música de combate (entrenadores, líderes, rivales…), también con 6 opciones. Se desbloquea al alcanzar el <b>nivel 8 de perfil</b>.",
    "guide.modes.infinite.title": "♾️ Modo Desafío Infinito",
    "guide.modes.infinite.desc": "Rondas sin límite con 6 opciones: la partida sigue mientras vayas acertando y termina en el primer fallo (normalmente no hay vidas de refuerzo). Cuanto más avances, más difícil y emocionante se vuelve. Se desbloquea con el logro «Viajero regional». Al completar el Modo Historia, además, pueden aparecer los Eventos Pokémon especiales también en este modo.",
    "guide.modes.story.title": "📖 Modo Historia",
    "guide.modes.story.desc": "Recorre todas las regiones en orden, de Kanto a Alola. En cada región juegas una fase de 10 rondas normales y, después, una fase de combate de 3 rondas contra un \"enemigo poderoso\". Aquí sí hay vidas ❤️: se recargan a 3 al empezar cada región y, al perderlas todas, termina la partida. La puntuación y la racha se acumulan durante todo el recorrido.",
    "guide.modes.minigames.title": "🕹️ Minijuegos",
    "guide.modes.minigames.desc": "Partidas cortas centradas en una categoría concreta: Centros Pokémon, Laboratorios, Bicicletas, Surf, Pantallas de Título, Calle Victoria, Openings del Anime, Pokémon Mundo Misterioso, Pokémon Colosseum/XD y Pokémon Ranger. La mayoría dura 5 rondas, salvo Pokémon Mundo Misterioso, que dura 10. Al igual que Fácil/Normal/Difícil/Combate, fallar no te elimina. Cada categoría se desbloquea al conseguir un logro concreto o al alcanzar un nivel de perfil determinado, según la categoría (puedes ver cuál en la propia pantalla de Minijuegos).",
    "guide.scoring.title": "🏆 Sistema de puntuación",
    "guide.scoring.speed.title": "⏱️ Velocidad de respuesta",
    "guide.scoring.speed.desc": "Cada acierto da entre 20 y 100 puntos base según lo rápido que respondas: cuanto antes contestes, más puntos consigues. A partir de 15 segundos, el acierto ya solo otorga el mínimo garantizado (20 pts).",
    "guide.scoring.streak.title": "🔥 Multiplicador de racha",
    "guide.scoring.streak.desc": "Encadenar aciertos aumenta tus puntos: desde la 2ª respuesta correcta seguida el multiplicador sube un x0.1 por cada racha (racha 2 = x1.1, racha 3 = x1.2…) hasta un tope de x2 a partir de una racha de 11.",
    "guide.scoring.lives.title": "❤️ Vidas",
    "guide.scoring.lives.desc": "Solo el Modo Historia tiene corazones desde el principio (3, y se recargan al pasar de región): cada fallo resta uno y, al perderlos todos, termina la partida. En Fácil, Normal, Difícil, Combate y Minijuegos no hay vidas: fallar solo corta la racha y siempre juegas la partida completa. El Desafío Infinito tampoco tiene vidas por defecto —cualquier fallo la acaba al instante—, salvo que el evento Venusaur te conceda alguna durante la partida.",
    "guide.achievements.title": "🏅 Logros y contenido desbloqueable",
    "guide.achievements.intro": "Puedes consultar todos tus logros en la pantalla de Logros, agrupados en 5 bloques: Progreso y rachas, Maestría y partidas perfectas, Sonidex, Modo Historia y Eventos Pokémon. Los logros marcados con una ⭐ desbloquean, además de la insignia, alguna función nueva:",
    "guide.achievements.modes.title": "🎮 Modos de juego",
    "guide.achievements.modes.desc": "El Modo Difícil se desbloquea al alcanzar el nivel 5 de perfil y el Modo Combate al alcanzar el nivel 8; el Desafío Infinito se desbloquea con el logro «Viajero regional» (jugar en todas las regiones).",
    "guide.achievements.minigames.title": "🕹️ Categorías de Minijuegos",
    "guide.achievements.minigames.desc": "Cada categoría de Minijuegos (Centros Pokémon, Laboratorios, Bicicletas, Surf, Pantallas de Título, Calle Victoria, Openings del Anime, Mundo Misterioso, Colosseum/XD, Ranger) se desbloquea al alcanzar un nivel de perfil concreto o al conseguir un logro concreto, según la categoría, indicado con un 🔒 en el botón mientras siga bloqueada.",
    "guide.achievements.events.title": "✨ Eventos Pokémon",
    "guide.achievements.events.desc": "Al completar el Modo Historia («Maestro de la Historia») se desbloquean los Eventos Pokémon sorpresa (Gengar, Hypno, Shiny, Porygon, Electrode, Blastoise, Venusaur…) también en el Desafío Infinito. Los logros de \"encuentro\" (por ejemplo, hacer aparecer 5 veces a un Pokémon) desbloquean, uno a uno, que ese Pokémon pueda pasear por el fondo del menú, y también el avatar de perfil de ese Pokémon.",
    "guide.achievements.sonidex.title": "🎼 Fichas de la Sonidex",
    "guide.achievements.sonidex.desc": "Además de logros individuales por número de fichas desbloqueadas (5, 10, 20, 50, 100, 200) y por completar la Sonidex de cada región, cada acierto que suma para desbloquear una ficha también cuenta para tus logros de aciertos totales (como «Melómano»), y algunos de esos logros abren categorías de Minijuegos.",
    "guide.sonidex.title": "🎼 Sonidex",
    "guide.sonidex.intro": "La Sonidex es tu colección de canciones. Cada canción tiene una ficha bloqueada 🔒 hasta que la aciertas <b>10 veces</b> en cualquier modo excepto el Fácil (en el que solo se adivina la región, no la canción). Al desbloquear una ficha podrás reproducir esa canción libremente desde la propia Sonidex.",
    "guide.sonidex.organization": "Las fichas están organizadas por región (Kanto, Johto, Hoenn, Sinnoh, Teselia, Kalos y Alola), Música de Combate y todas las categorías de Minijuegos (Centros Pokémon, Laboratorios, Bicicletas, Surf, Pantallas de Título, Calle Victoria, Openings del Anime, Pokémon Mundo Misterioso, Pokémon Colosseum/XD y Pokémon Ranger). La barra de progreso y el contador «X / Y canciones desbloqueadas» tienen en cuenta absolutamente todas las canciones del juego.",
    "guide.sonidex.cities.title": "🏙️ ¿Por qué no aparecen todas las ciudades?",
    "guide.sonidex.cities.desc": "Algunas ciudades comparten exactamente la misma melodía dentro del juego original, así que en la Sonidex solo aparece una ficha para representarlas y no una por cada ciudad. Por ejemplo, Ciudad Azulona y Ciudad Fucsia suenan con la misma canción, por lo que comparten una única ficha en lugar de tener una cada una.",
    "guide.profile.title": "🧑‍🎤 Perfil, experiencia y avatares",
    "guide.profile.xp.title": "📈 Experiencia y nivel",
    "guide.profile.xp.desc": "Todos los puntos que ganas jugando (aciertos, rachas, eventos especiales…) se suman también como experiencia de tu perfil. Al acumular suficiente experiencia subes de nivel, y cada nivel necesita algo más que el anterior. Subir de nivel es lo que va desbloqueando, poco a poco, el Modo Difícil, el Modo Combate, varias categorías de Minijuegos y algunos avatares (mira los requisitos exactos en cada pantalla de desbloqueo).",
    "guide.profile.avatars.title": "🖼️ Avatares",
    "guide.profile.avatars.desc": "Puedes personalizar tu perfil eligiendo un avatar entre un catálogo muy amplio de Pokémon. Muchos están disponibles desde el principio; el resto se va desbloqueando al subir de nivel, al conseguir logros de \"encuentro\" con un Pokémon de un Evento Pokémon (5 apariciones), o al completar una partida perfecta en ciertos modos y minijuegos. Toca tu avatar en la barra de perfil para cambiarlo o para cambiar tu nombre de jugador.",
    "guide.leaderboard.title": "🏆 Clasificación Global",
    "guide.leaderboard.intro": "Desde el botón 🏆 de la pantalla principal puedes consultar cuatro tablas de clasificación online, con los mejores jugadores del mundo: nivel de perfil, mejor puntuación en Desafío Infinito, mejor puntuación en Modo Historia y mejor puntuación en Modo Difícil.",
    "guide.leaderboard.update": "Tu puesto se actualiza en automático cada vez que superas tu propio récord anterior en alguna de esas cuatro categorías; no hace falta enviarlo a mano. Apareces con el nombre y el avatar de tu perfil, así que si quieres identificarte en la clasificación, personalízalos desde la barra de perfil.",

    "achievements.title": "🏅 Logros",
    "achievements.streaksTitle": "🔥 Rachas máximas",
    "achievements.progress": "{n} / {total} desbloqueados",

    "sonidex.title": "🎼 Sonidex",
    "sonidex.note": "🔓 Acierta una misma canción 10 veces (en cualquier modo excepto el Fácil) para desbloquear su ficha.",
    "sonidex.progress": "{n} / {total} canciones desbloqueadas",
    "sonidex.progressShort": "{n} / {total} desbloqueadas",
    "sonidex.changeLanguage": "Cambiar idioma del opening",

    "leaderboard.title": "🏆 Clasificaciones",
    "leaderboard.subtitle": "Los 50 mejores de cada categoría",
    "leaderboard.yourRecords": "🙋 Tus récords",
    "leaderboard.level": "🧑 Nivel de Jugador",
    "leaderboard.infinite": "♾️ Desafío Infinito",
    "leaderboard.story": "📖 Modo Historia",
    "leaderboard.hard": "🔴 Modo Difícil",
    "leaderboard.combat": "⚔️ Modo Combate",
    "leaderboard.regionsGroup": "🗺️ Modo Normal, por región",
    "leaderboard.top50": "🌍 Top 50 global",
    "leaderboard.tab.level": "🧑 Nivel",
    "leaderboard.tab.infinite": "♾️ Infinito",
    "leaderboard.tab.story": "📖 Historia",
    "leaderboard.tab.hard": "🔴 Difícil",
    "leaderboard.tab.combat": "⚔️ Combate",
    "leaderboard.tab.regions": "🗺️ Regiones",
    "leaderboard.loading": "Cargando clasificación…",
    "leaderboard.error": "⚠️ No se ha podido cargar la clasificación. Inténtalo más tarde.",
    "leaderboard.empty": "Todavía no hay clasificación global disponible.",
    "leaderboard.viewProfile": "Ver perfil de {name}",
    "publicProfile.scoresTitle": "🏆 Puntuaciones",

    "quiz.loading": "Cargando…",
    "quiz.hint": "👁️ Pista visual",
    "quiz.hintUsed": "👁️ Pista usada",
    "quiz.hintCost": "-50% puntos",
    "quiz.next": "▶ Siguiente Ronda",
    "quiz.modeEasy": "Modo: Fácil · Adivina la región",
    "quiz.modeHard": "Modo: Difícil · 6 opciones · 10s por ronda",
    "quiz.hardTimerUnit": "s",
    "quiz.modeInfinite": "Modo: Desafío Infinito · 6 opciones · Un fallo termina la partida",
    "quiz.modeCombat": "Modo: Combate · 6 opciones",
    "quiz.modeNormalRegion": "Modo: Normal · Región: {region}",
    "quiz.modeOther": "Modo: Minijuegos · {game}",
    "quiz.modeStoryEnemy": "📖 Modo Historia · ⚔️ Enemigo poderoso",
    "quiz.modeStoryRegion": "📖 Modo Historia · Región: {region}",
    "quiz.questionRegion": "🌍 ¿A qué región pertenece esta canción?",
    "quiz.questionSong": "🎶 ¿Qué canción está sonando?",
    "quiz.playing": "▶ Reproduciendo...",
    "quiz.tapToPlay": "▶ Toca la pantalla para reproducir",
    "quiz.chanseySecondChance": "💗 ¡Chansey te da otra oportunidad!",
    "quiz.hardTimeUp": "⏱️ ¡Tiempo agotado!",

    "result.title": "¡Fin del juego!",
    "result.scoreLabel": "Puntuación:",
    "result.restart": "🔄 Jugar de nuevo",
    "result.exit": "🚪 Salir",
    "result.attempt": "¡Buen intento!",
    "result.master": "¡Maestro Pokémon!",
    "result.great": "¡Muy bien!",
    "result.good": "¡Buen trabajo!",
    "result.infiniteScore": "{n} rondas superadas · 💰 {score} pts",
    "result.roundsScore": "{n} / {total} · 💰 {score} pts",

    "confirm.leaveStoryTitle": "¿Salir del Modo Historia?",
    "confirm.leaveStoryBody": "Si sales ahora perderás el progreso de esta partida.<br>La próxima vez tendrás que empezar de nuevo desde Kanto.",
    "confirm.keepPlaying": "🎮 Seguir jugando",
    "confirm.leaveAnyway": "🚪 Salir igualmente",

    "endlessInfo.title": "¡Modo infinito activado!",
    "endlessInfo.body": "Las rondas serán infinitas, pero <strong>un solo fallo terminará la partida</strong>.",
    "endlessInfo.dontShowAgain": "No volver a mostrar",
    "endlessInfo.ok": "👍 ¡Entendido!",

    "toast.levelUpLabel": "Subida de nivel",
    "toast.levelUpTitle": "¡Subiste al nivel {n}!",
    "toast.modeUnlockedLabel": "Modo desbloqueado",
    "toast.minigameUnlockedLabel": "Minijuego desbloqueado",
    "toast.unlockedTitle": "¡{name} desbloqueado!",
    "toast.avatarUnlockedLabel": "Avatar desbloqueado",
    "toast.newAvatarTitle": "¡Nuevo avatar disponible: {name}!",
    "toast.soundexUnlockedLabel": "Ficha desbloqueada",
    "toast.soundexUnlockedTitle": "¡Ficha desbloqueada en Sonidex!: {title}",
    "toast.featureUnlockedLabel": "Función desbloqueada",
    "toast.pokeEventsUnlockedTitle": "¡Eventos Pokémon desbloqueados en el Desafío Infinito!",
    "toast.newCursorTitle": "¡Nuevo puntero disponible: {name}!",
    "toast.newHillEncounterLabel": "Nuevo encuentro",
    "toast.newHillEncounterTitle": "¡{name} ahora pasea por las colinas!",
    "toast.achievementUnlockedLabel": "Logro desbloqueado",

    "pokeEvent.tag": "✨ Evento Pokémon",

    "feature.gameMode": "modo de juego",
    "feature.minigameCategory": "categoría de Minijuegos",
    "feature.pokeEventsInInfinite": "Eventos Pokémon",
    "feature.pokeEventsInInfiniteType": "en el Desafío Infinito",
    "feature.hillPokemon": "Pokémon del fondo",
    "feature.hillPokemonName": "{name} en las colinas",
    "feature.avatarType": "avatar de perfil",
    "feature.avatarName": "Avatar: {name}",
    "feature.avatarGeneric": "un avatar",
    "feature.cursorName": "Puntero: {name}",
    "feature.cursorType": "estilo de puntero",
    "feature.unlocksLabel": "desbloquea:",
    "achFeature.closeHint": "Toca fuera de este recuadro para cerrar",

    "story.subtitleMain": "MODO HISTORIA",
    "story.tapStart": "Toca la pantalla para comenzar",
    "story.enemyTitle": "Ha aparecido un enemigo poderoso",
    "story.subtitleCombat": "COMBATE",
    "story.tapFight": "Toca la pantalla para luchar",
    "story.gameOverTitle": "Game Over",
    "story.gameOverDesc": "Has perdido tus 3 vidas. Vuelve a intentarlo desde Kanto.",
    "story.completeTitle": "¡Modo Historia Completado!",

    // Nombres de región (solo hace falta listar aquí las que difieren
    // del nombre interno usado como clave de datos — ver
    // regionDisplayName más abajo). En español el nombre interno YA es
    // el correcto, así que este idioma no necesita ninguna entrada
    // "region.*".
  },

  en: {
    "splash.tap": "Tap the screen to start",
    "splash.subtitle": "🎵 PokéQuiz · Music Edition",
    "brand.radioTrigal": "Radio Trigal",
    "brand.fm": "FM",

    "common.close": "Close",
    "common.trainerDefault": "Trainer",

    "pokeEventsInfo.btnAria": "Pokémon Events info",
    "pokeEventsInfo.title": "🎲 Pokémon Events",
    "pokeEventsInfo.subtitle": "During a round, one of these Pokémon may randomly appear with a special effect:",

    "profileSetup.title": "Welcome, Trainer!",
    "profileSetup.desc": "Before you start, choose your trainer name and profile picture.",
    "profileSetup.namePlaceholder": "Your trainer name",
    "profileSetup.avatarLabel": "Choose your profile picture",
    "profileSetup.confirm": "✔ Start playing!",
    "profileSetup.checking": "Checking…",
    "profileSetup.nameTaken": "That trainer name is already taken. Choose another one.",

    "profile.editNameTitle": "Change name",
    "profile.changeAvatarBtn": "🖼️ Change profile picture",
    "profile.levelLabel": "Level",
    "profile.xpLabel": "{into} / {need} XP to level {next}",

    "profile.stats.title": "Statistics",
    "profile.stats.totalPoints": "💰 Total points",
    "profile.stats.achievementsUnlocked": "🏅 Achievements unlocked",
    "profile.stats.gamesPlayed": "🎮 Games played",
    "profile.stats.correctAnswers": "✅ Correct answers",
    "profile.stats.bestStreak": "🔥 Best streak",
    "profile.stats.infiniteRecord": "♾️ Endless Challenge record",
    "profile.stats.storyRecord": "📖 Story Mode record",
    "profile.stats.hardRecord": "🔴 Hard Mode record",
    "profile.stats.combatRecord": "⚔️ Battle Mode record",
    "profile.stats.regionRecordsTitle": "🗺️ Records by region",
    "profile.stats.sonidexUnlocked": "🎼 Soundex entries unlocked",
    "common.pts": "pts",

    "lock.badge": "Locked",
    "lock.levelReqBadge": "Unlocks at profile level {level}",
    "lock.achievementReqBadge": "Unlocks with the “{title}” achievement",
    "lock.levelReqToast": "Reach profile level {level} to unlock",
    "lock.achievementReqToast": "Get “{title}” to unlock",
    "avatar.lockedTitle": "{name} (locked — requires {req})",
    "avatar.lockedToast": "Get {req} to unlock this avatar",
    "avatar.lockReqLevel": "profile level {level}",
    "avatar.lockReqAchievement": "the “{title}” achievement",
    "avatar.lockReqUnknown": "an achievement",

    "streaks.easyMode": "Easy mode",
    "streaks.hardMode": "Hard mode",
    "streaks.combatMode": "Battle mode",
    "streaks.infiniteMode": "Endless Challenge",
    "streaks.normalByRegion": "Normal mode · by region",
    "streaks.easyLabel": "🟢 Easy",
    "streaks.hardLabel": "🔴 Hard",
    "streaks.combatLabel": "⚔️ Battle",
    "streaks.infiniteLabel": "♾️ Challenge",

    "ach.starBadgeTooltip": "Unlocks a special feature",
    "ach.unlockedOn": "Unlocked on {date}",

    "story.regionCompleted": "{region} Completed",

    "header.back": "Back",
    "header.points": "Points",
    "header.round": "Round",
    "header.streak": "Streak",

    "home.tagline": "Your Pokémon radio station",
    "home.play.title": "Play",
    "home.play.desc": "5 modes and Minigames",
    "home.story.title": "Story Mode",
    "home.story.desc": "Journey through every region, from Kanto to Alola",
    "home.achievements.title": "Achievements",
    "home.sonidex.title": "Soundex",
    "home.leaderboard.title": "Leaderboards",
    "home.options.title": "Options",
    "home.achSummaryDefault": "Check your unlocked achievements",
    "home.sonidexSummaryDefault": "Unlock songs by guessing them 10 times",

    "modes.easy.title": "Easy Mode",
    "modes.easy.desc": "Just guess the region (Kanto, Johto, …)",
    "modes.normal.title": "Normal Mode",
    "modes.normal.desc": "Pick a region first, then guess the song",
    "modes.hard.title": "Hard Mode",
    "modes.hard.desc": "6 options · songs from any game · 10s per round",
    "modes.combat.title": "Battle Mode",
    "modes.combat.desc": "6 options · Guess the battle songs",
    "modes.infinite.title": "Endless Challenge",
    "modes.infinite.desc": "6 options · No round limit · one mistake ends the run",
    "modes.other.title": "Minigames",
    "modes.other.desc": "Labs · Bicycle · Other Sagas",
    "modes.endlessToggle.title": "Endless toggle: no round limit, one mistake ends the run",

    "region.title": "Normal Mode",
    "region.subtitle": "Choose a region. Songs and answers will only be from that region.",

    "other.title": "Minigames",
    "other.subtitle": "Pick a category. Only songs from that option will play.",
    "other.centro.title": "Pokémon Center",
    "other.centro.desc": "Only Pokémon Center songs and options",
    "other.lab.title": "Labs",
    "other.lab.desc": "Only Lab songs and options",
    "other.bike.title": "Bicycles",
    "other.bike.desc": "Only Bicycle songs and options",
    "other.surf.title": "Surf",
    "other.surf.desc": "Only Surf songs and options",
    "other.titlescreens.title": "Title Screens",
    "other.titlescreens.desc": "Only Title Screen songs and options",
    "other.callevictoria.title": "Victory Road",
    "other.callevictoria.desc": "Only Victory Road songs and options",
    "other.openinganime.title": "Anime Openings",
    "other.openinganime.desc": "Only Anime Opening songs and options",
    "other.mysterydungeon.title": "Pokémon Mystery Dungeon",
    "other.mysterydungeon.desc": "Only Mystery Dungeon songs and options",
    "other.colosseum.title": "Pokémon Colosseum / XD",
    "other.colosseum.desc": "Only Colosseum/XD songs and options",
    "other.ranger.title": "Pokémon Ranger",
    "other.ranger.desc": "Only Ranger songs and options",

    // Anime Openings pre-screen (dub choice). In practice only reachable when
    // playing in Spanish (see the [data-other] listener in game.js), but kept
    // translated too since every data-i18n key must exist in both languages.
    "openingsLang.title": "Anime Openings",
    "openingsLang.subtitle": "Which dub do you want the songs in?",
    "openingsLang.spain.title": "European Spanish",
    "openingsLang.spain.desc": "The openings as aired in Spain",
    "openingsLang.latino.title": "Latin American Spanish",
    "openingsLang.latino.desc": "The openings with the Latin American dub",

    "options.title": "Options",
    "options.subtitle": "Adjust audio and appearance.",
    "options.music.label": "Music",
    "options.music.desc": "Song volume",
    "options.sfx.label": "Sound effects",
    "options.sfx.desc": "Correct/wrong answer volume",
    "options.dark.label": "Dark Mode",
    "options.dark.desc": "Darker interface for low light",
    "options.toggleOn": "On",
    "options.toggleOff": "Off",
    "options.saveNote": "💾 These options are saved automatically.",
    "options.graphics.title": "🖼️ Graphics options",
    "options.bg.label": "Animated background",
    "options.bg.desc": "Sky, clouds and walking Pokémon in the background",
    "options.particles.label": "Particle effects",
    "options.particles.desc": "Stars and sparks on correct answers or achievements",
    "options.cursor.label": "🖱️ Mouse cursor",
    "options.cursor.desc": "Swap it for a sprite you've unlocked with achievements",
    "options.cursor.normalName": "Normal",
    "cursorStyle.poke_flute.name": "Poké Flute",
    "cursorStyle.poke_ball.name": "Poké Ball",
    "cursorStyle.super_rod.name": "Super Rod",
    "cursorStyle.rare_candy.name": "Rare Candy",
    "cursorStyle.shuckle.name": "Shuckle",
    "cursorStyle.cosmog.name": "Cosmog",
    "cursorStyle.togedemaru.name": "Togedemaru",
    "cursorStyle.zygarde.name": "Zygarde",
    "cursorStyle.dedenne.name": "Dedenne",
    "cursorStyle.emolga.name": "Emolga",
    "cursorStyle.rotom.name": "Rotom",
    "cursorStyle.pachirisu.name": "Pachirisu",
    "cursorStyle.plusle.name": "Plusle",
    "cursorStyle.pichu.name": "Pichu",
    "cursorStyle.mew.name": "Mew",
    "cursorStyle.bulbasaur.name": "Bulbasaur",
    "cursorStyle.squirtle.name": "Squirtle",
    "cursorStyle.charmander.name": "Charmander",
    "cursorStyle.meowth.name": "Meowth",
    "cursorStyle.charizard_y.name": "Charizard Y",
    "cursorStyle.charizard_x.name": "Charizard X",
    "cursorStyle.pikachu.name": "Pikachu",
    "cursorStyle.spinda.name": "Spinda",
    "cursorStyle.ludicolo.name": "Ludicolo",
    "cursorStyle.manaphy.name": "Manaphy",
    "cursorStyle.magnemite.name": "Magnemite",
    "cursorStyle.chansey.name": "Chansey",
    "cursorStyle.rayquaza.name": "Rayquaza",
    "cursorStyle.jigglypuff.name": "Jigglypuff",
    "cursorStyle.regigigas.name": "Regigigas",
    "cursorStyle.rayquaza_shiny.name": "Shiny Rayquaza",
    "cursorStyle.meloetta.name": "Meloetta",
    "cursorStyle.lockedTitle": "{name} (locked — requires {req})",
    "cursorStyle.lockedToast": "Get {req} to unlock this cursor",
    "options.language.title": "🌐 Language",
    "options.language.desc": "Language for menus, achievements, Pokémon Events and the Game Guide",
    "options.language.es": "Español",
    "options.language.en": "English",
    "options.language.ja": "日本語",
    "options.guide.label": "📘 Game Guide",
    "options.guide.desc": "Modes, scoring, achievements and Soundex explained",

    "guide.header.title": "📘 Game Guide",
    "guide.header.subtitle": "Everything you need to know to master Radio Trigal FM.",
    "guide.modes.title": "🎮 Game modes",
    "guide.modes.easy.title": "🟢 Easy Mode",
    "guide.modes.easy.desc": "A song plays and you just have to guess which <b>region</b> it's from (Kanto, Johto, Hoenn…). It's the simplest mode: perfect for getting started. Games last 10 rounds; a wrong answer doesn't eliminate you, it just breaks your streak, so you always play the full 10 rounds.",
    "guide.modes.normal.title": "🔵 Normal Mode",
    "guide.modes.normal.desc": "First you pick a region, then you have to guess the <b>exact title</b> of the song among several options from that same region. 10 rounds; like in Easy, a wrong answer doesn't end the game, it just breaks your streak.",
    "guide.modes.hard.title": "🔴 Hard Mode",
    "guide.modes.hard.desc": "Songs can be from <b>any region</b>, chosen among 6 options, and you only have 10 seconds per round to answer. 10 rounds, no elimination on a wrong answer. Unlocked at <b>profile level 5</b>.",
    "guide.modes.battle.title": "⚔️ Battle Mode",
    "guide.modes.battle.desc": "Same as Hard but only with battle music (trainers, gym leaders, rivals…), also with 6 options. Unlocked at <b>profile level 8</b>.",
    "guide.modes.infinite.title": "♾️ Infinite Challenge Mode",
    "guide.modes.infinite.desc": "Unlimited rounds with 6 options: the game continues as long as you keep answering correctly and ends on the first wrong answer (there are normally no bonus lives). The further you get, the harder and more exciting it becomes. Unlocked with the «Regional Traveler» achievement. After completing Story Mode, the special Pokémon Events can also appear in this mode.",
    "guide.modes.story.title": "📖 Story Mode",
    "guide.modes.story.desc": "Travel through every region in order, from Kanto to Alola. In each region you play a 10-round normal phase and then a 3-round battle phase against a \"powerful enemy\". This mode does have lives ❤️: they recharge to 3 at the start of each region, and losing them all ends the game. Score and streak carry over across the whole journey.",
    "guide.modes.minigames.title": "🕹️ Minigames",
    "guide.modes.minigames.desc": "Short games focused on one specific category: Pokémon Centers, Labs, Bicycles, Surfing, Title Screens, Victory Road, Anime Openings, Pokémon Mystery Dungeon, Pokémon Colosseum/XD and Pokémon Ranger. Most last 5 rounds, except Pokémon Mystery Dungeon, which lasts 10. Like Easy/Normal/Hard/Battle, a wrong answer doesn't eliminate you. Each category unlocks by earning a specific achievement or reaching a certain profile level, depending on the category (you can check which on the Minigames screen itself).",
    "guide.scoring.title": "🏆 Scoring system",
    "guide.scoring.speed.title": "⏱️ Answer speed",
    "guide.scoring.speed.desc": "Each correct answer gives between 20 and 100 base points depending on how fast you answer: the sooner you answer, the more points you get. From 15 seconds onward, a correct answer only grants the guaranteed minimum (20 pts).",
    "guide.scoring.streak.title": "🔥 Streak multiplier",
    "guide.scoring.streak.desc": "Chaining correct answers boosts your points: from the 2nd correct answer in a row, the multiplier rises by x0.1 per streak (streak 2 = x1.1, streak 3 = x1.2…) up to a cap of x2 from a streak of 11 onward.",
    "guide.scoring.lives.title": "❤️ Lives",
    "guide.scoring.lives.desc": "Only Story Mode has hearts from the start (3, recharging when you move to a new region): each wrong answer removes one, and losing them all ends the game. In Easy, Normal, Hard, Battle and Minigames there are no lives: a wrong answer only breaks your streak and you always play the full game. Infinite Challenge also has no lives by default —any wrong answer ends it instantly— unless the Venusaur event grants you some during the game.",
    "guide.achievements.title": "🏅 Achievements and unlockable content",
    "guide.achievements.intro": "You can check all your achievements on the Achievements screen, grouped into 5 blocks: Progress and streaks, Mastery and perfect games, Sonidex, Story Mode and Pokémon Events. Achievements marked with a ⭐ unlock, besides the badge, some new feature:",
    "guide.achievements.modes.title": "🎮 Game modes",
    "guide.achievements.modes.desc": "Hard Mode unlocks at profile level 5 and Battle Mode at level 8; Infinite Challenge unlocks with the «Regional Traveler» achievement (playing in every region).",
    "guide.achievements.minigames.title": "🕹️ Minigame categories",
    "guide.achievements.minigames.desc": "Each Minigame category (Pokémon Centers, Labs, Bicycles, Surfing, Title Screens, Victory Road, Anime Openings, Mystery Dungeon, Colosseum/XD, Ranger) unlocks at a specific profile level or by earning a specific achievement, depending on the category, shown with a 🔒 on the button while still locked.",
    "guide.achievements.events.title": "✨ Pokémon Events",
    "guide.achievements.events.desc": "Completing Story Mode («Story Master») unlocks the surprise Pokémon Events (Gengar, Hypno, Shiny, Porygon, Electrode, Blastoise, Venusaur…) in Infinite Challenge too. The \"encounter\" achievements (for example, making a Pokémon appear 5 times) unlock, one by one, that Pokémon wandering the menu background, as well as that Pokémon's profile avatar.",
    "guide.achievements.sonidex.title": "🎼 Sonidex entries",
    "guide.achievements.sonidex.desc": "Besides individual achievements for the number of unlocked entries (5, 10, 20, 50, 100, 200) and for completing each region's Sonidex, every correct answer that counts toward unlocking an entry also counts toward your total correct-answer achievements (like «Music Lover»), and some of those achievements open up Minigame categories.",
    "guide.sonidex.title": "🎼 Sonidex",
    "guide.sonidex.intro": "The Sonidex is your song collection. Each song has a locked 🔒 entry until you get it right <b>10 times</b> in any mode except Easy (where you only guess the region, not the song). Unlocking an entry lets you play that song freely from the Sonidex itself.",
    "guide.sonidex.organization": "Entries are organized by region (Kanto, Johto, Hoenn, Sinnoh, Unova, Kalos and Alola), Battle Music and every Minigame category (Pokémon Centers, Labs, Bicycles, Surfing, Title Screens, Victory Road, Anime Openings, Pokémon Mystery Dungeon, Pokémon Colosseum/XD and Pokémon Ranger). The progress bar and the «X / Y songs unlocked» counter take into account absolutely every song in the game.",
    "guide.sonidex.cities.title": "🏙️ Why don't all cities show up?",
    "guide.sonidex.cities.desc": "Some cities share the exact same tune in the original game, so the Sonidex only shows one entry to represent them instead of one per city. For example, Cerulean City and Fuchsia City play the same song, so they share a single entry instead of having one each.",
    "guide.profile.title": "🧑‍🎤 Profile, experience and avatars",
    "guide.profile.xp.title": "📈 Experience and level",
    "guide.profile.xp.desc": "All the points you earn while playing (correct answers, streaks, special events…) also add up as profile experience. Once you gather enough experience you level up, and each level needs a bit more than the last. Leveling up is what gradually unlocks Hard Mode, Battle Mode, several Minigame categories and some avatars (check the exact requirements on each unlock screen).",
    "guide.profile.avatars.title": "🖼️ Avatars",
    "guide.profile.avatars.desc": "You can customize your profile by picking an avatar from a very wide catalog of Pokémon. Many are available from the start; the rest unlock as you level up, by earning \"encounter\" achievements with a Pokémon Event Pokémon (5 appearances), or by completing a perfect game in certain modes and minigames. Tap your avatar in the profile bar to change it or to change your player name.",
    "guide.leaderboard.title": "🏆 Global Leaderboard",
    "guide.leaderboard.intro": "From the 🏆 button on the main screen you can check four online leaderboards with the best players in the world: profile level, best Infinite Challenge score, best Story Mode score and best Hard Mode score.",
    "guide.leaderboard.update": "Your position updates automatically every time you beat your own previous record in any of those four categories; there's no need to submit it by hand. You show up with your profile's name and avatar, so if you want to be recognizable on the leaderboard, customize them from the profile bar.",

    "achievements.title": "🏅 Achievements",
    "achievements.streaksTitle": "🔥 Best streaks",
    "achievements.progress": "{n} / {total} unlocked",

    "sonidex.title": "🎼 Soundex",
    "sonidex.note": "🔓 Guess the same song 10 times (in any mode except Easy) to unlock its entry.",
    "sonidex.progress": "{n} / {total} songs unlocked",
    "sonidex.progressShort": "{n} / {total} unlocked",
    "sonidex.changeLanguage": "Change opening language",

    "leaderboard.title": "🏆 Leaderboards",
    "leaderboard.subtitle": "The top 50 of each category",
    "leaderboard.yourRecords": "🙋 Your records",
    "leaderboard.level": "🧑 Player Level",
    "leaderboard.infinite": "♾️ Endless Challenge",
    "leaderboard.story": "📖 Story Mode",
    "leaderboard.hard": "🔴 Hard Mode",
    "leaderboard.combat": "⚔️ Battle Mode",
    "leaderboard.regionsGroup": "🗺️ Normal Mode, by region",
    "leaderboard.top50": "🌍 Global Top 50",
    "leaderboard.tab.level": "🧑 Level",
    "leaderboard.tab.infinite": "♾️ Endless",
    "leaderboard.tab.story": "📖 Story",
    "leaderboard.tab.hard": "🔴 Hard",
    "leaderboard.tab.combat": "⚔️ Battle",
    "leaderboard.tab.regions": "🗺️ Regions",
    "leaderboard.loading": "Loading leaderboard…",
    "leaderboard.error": "⚠️ Couldn't load the leaderboard. Try again later.",
    "leaderboard.empty": "There's no global leaderboard yet.",
    "leaderboard.viewProfile": "View {name}'s profile",
    "publicProfile.scoresTitle": "🏆 Scores",

    "quiz.loading": "Loading…",
    "quiz.hint": "👁️ Visual hint",
    "quiz.hintUsed": "👁️ Hint used",
    "quiz.hintCost": "-50% points",
    "quiz.next": "▶ Next Round",
    "quiz.modeEasy": "Mode: Easy · Guess the region",
    "quiz.modeHard": "Mode: Hard · 6 options · 10s per round",
    "quiz.hardTimerUnit": "s",
    "quiz.modeInfinite": "Mode: Endless Challenge · 6 options · One mistake ends the run",
    "quiz.modeCombat": "Mode: Battle · 6 options",
    "quiz.modeNormalRegion": "Mode: Normal · Region: {region}",
    "quiz.modeOther": "Mode: Minigames · {game}",
    "quiz.modeStoryEnemy": "📖 Story Mode · ⚔️ Powerful enemy",
    "quiz.modeStoryRegion": "📖 Story Mode · Region: {region}",
    "quiz.questionRegion": "🌍 Which region does this song belong to?",
    "quiz.questionSong": "🎶 What song is playing?",
    "quiz.playing": "▶ Playing...",
    "quiz.tapToPlay": "▶ Tap the screen to play",
    "quiz.chanseySecondChance": "💗 Chansey gives you another chance!",
    "quiz.hardTimeUp": "⏱️ Time's up!",

    "result.title": "Game over!",
    "result.scoreLabel": "Score:",
    "result.restart": "🔄 Play again",
    "result.exit": "🚪 Exit",
    "result.attempt": "Good effort!",
    "result.master": "Pokémon Master!",
    "result.great": "Great job!",
    "result.good": "Nice work!",
    "result.infiniteScore": "{n} rounds cleared · 💰 {score} pts",
    "result.roundsScore": "{n} / {total} · 💰 {score} pts",

    "confirm.leaveStoryTitle": "Leave Story Mode?",
    "confirm.leaveStoryBody": "If you leave now you'll lose this run's progress.<br>Next time you'll have to start over from Kanto.",
    "confirm.keepPlaying": "🎮 Keep playing",
    "confirm.leaveAnyway": "🚪 Leave anyway",

    "endlessInfo.title": "Endless mode on!",
    "endlessInfo.body": "Rounds will be endless, but <strong>a single mistake ends the run</strong>.",
    "endlessInfo.dontShowAgain": "Don't show this again",
    "endlessInfo.ok": "👍 Got it!",

    "toast.levelUpLabel": "Level up",
    "toast.levelUpTitle": "You reached level {n}!",
    "toast.modeUnlockedLabel": "Mode unlocked",
    "toast.minigameUnlockedLabel": "Minigame unlocked",
    "toast.unlockedTitle": "{name} unlocked!",
    "toast.avatarUnlockedLabel": "Avatar unlocked",
    "toast.newAvatarTitle": "New avatar available: {name}!",
    "toast.soundexUnlockedLabel": "Entry unlocked",
    "toast.soundexUnlockedTitle": "Soundex entry unlocked!: {title}",
    "toast.featureUnlockedLabel": "Feature unlocked",
    "toast.pokeEventsUnlockedTitle": "Pokémon Events unlocked in the Endless Challenge!",
    "toast.newCursorTitle": "New cursor available: {name}!",
    "toast.newHillEncounterLabel": "New encounter",
    "toast.newHillEncounterTitle": "{name} now wanders the hills!",
    "toast.achievementUnlockedLabel": "Achievement unlocked",

    "pokeEvent.tag": "✨ Pokémon Event",

    "feature.gameMode": "game mode",
    "feature.minigameCategory": "Minigame category",
    "feature.pokeEventsInInfinite": "Pokémon Events",
    "feature.pokeEventsInInfiniteType": "in the Endless Challenge",
    "feature.hillPokemon": "background Pokémon",
    "feature.hillPokemonName": "{name} on the hills",
    "feature.avatarType": "profile avatar",
    "feature.avatarName": "Avatar: {name}",
    "feature.avatarGeneric": "an avatar",
    "feature.cursorName": "Cursor: {name}",
    "feature.cursorType": "cursor style",
    "feature.unlocksLabel": "unlocks:",
    "achFeature.closeHint": "Tap outside this box to close",

    "story.subtitleMain": "STORY MODE",
    "story.tapStart": "Tap the screen to begin",
    "story.enemyTitle": "A powerful enemy has appeared",
    "story.subtitleCombat": "BATTLE",
    "story.tapFight": "Tap the screen to fight",
    "story.gameOverTitle": "Game Over",
    "story.gameOverDesc": "You lost all 3 lives. Try again from Kanto.",
    "story.completeTitle": "Story Mode Complete!",

    // ── Logros (ACHIEVEMENTS en game.js): solo estas claves en
    // inglés, el español ya vive en el propio catálogo (a.title/a.desc)
    // y se usa vía tData() para no duplicarlo — ver la cabecera de este
    // fichero y tData() más abajo. ──
    "achv.first_correct.title": "First correct answer",
    "achv.first_correct.desc": "Get your first correct answer.",
    "achv.streak_3.title": "Nice streak",
    "achv.streak_3.desc": "Reach a streak of 3 correct answers in a row.",
    "achv.streak_5.title": "On a roll",
    "achv.streak_5.desc": "Reach a streak of 5 correct answers in a row.",
    "achv.streak_5_thrice.title": "Repeat streaker",
    "achv.streak_5_thrice.desc": "Reach a streak of 5 correct answers in a row on 3 separate occasions.",
    "achv.streak_20.title": "Living legend",
    "achv.streak_20.desc": "Reach a streak of 20 correct answers in a row.",
    "achv.streak_30.title": "Unstoppable streak",
    "achv.streak_30.desc": "Reach a streak of 30 correct answers in a row.",
    "achv.streak_50.title": "Legendary streak",
    "achv.streak_50.desc": "Reach a streak of 50 correct answers in a row.",
    "achv.streak_100.title": "Mythic streak",
    "achv.streak_100.desc": "Reach a streak of 100 correct answers in a row.",
    "achv.perfect_normal_region.title": "Perfect region",
    "achv.perfect_normal_region.desc": "Complete a perfect run in any region of Normal mode.",
    "achv.perfect_easy.title": "Perfect Easy",
    "achv.perfect_easy.desc": "Complete a perfect run in Easy mode.",
    "achv.hard_correct_8.title": "Challenge cleared",
    "achv.hard_correct_8.desc": "Get 8 or more correct answers in a Hard mode game.",
    "achv.perfect_hard.title": "Musical genius",
    "achv.perfect_hard.desc": "Complete a perfect run in Hard mode.",
    "achv.perfect_regions_normal_5.title": "Regional specialist",
    "achv.perfect_regions_normal_5.desc": "Complete 5 regions in Normal mode with 100% accuracy.",
    "achv.correct_10.title": "Good ear",
    "achv.correct_10.desc": "Answer 10 questions correctly.",
    "achv.correct_20.title": "Trained ear",
    "achv.correct_20.desc": "Answer 20 questions correctly.",
    "achv.correct_50.title": "Music connoisseur",
    "achv.correct_50.desc": "Answer 50 questions correctly.",
    "achv.correct_100.title": "Music lover",
    "achv.correct_100.desc": "Answer 100 questions correctly.",
    "achv.correct_250.title": "Music fanatic",
    "achv.correct_250.desc": "Answer 250 questions correctly.",
    "achv.correct_500.title": "Musical encyclopedia",
    "achv.correct_500.desc": "Answer 500 questions correctly.",
    "achv.games_10.title": "Enthusiast",
    "achv.games_10.desc": "Play 10 games.",
    "achv.games_20.title": "Regular player",
    "achv.games_20.desc": "Play 20 games.",
    "achv.games_30.title": "Dedicated trainer",
    "achv.games_30.desc": "Play 30 games.",
    "achv.games_50.title": "Veteran",
    "achv.games_50.desc": "Play 50 games.",
    "achv.games_100.title": "Radio Trigal FM Legend",
    "achv.games_100.desc": "Play 100 games.",
    "achv.all_modes.title": "Explorer",
    "achv.all_modes.desc": "Complete at least one game in Easy, Normal and Hard modes.",
    "achv.all_regions.title": "Regional traveler",
    "achv.all_regions.desc": "Complete at least one game in every available region.",
    "achv.perfect_combat.title": "Battle ace",
    "achv.perfect_combat.desc": "Complete a perfect run in Combat mode.",
    "achv.perfect_colosseum_xd.title": "Perfect shadow",
    "achv.perfect_colosseum_xd.desc": "Complete a perfect run in the Pokémon Colosseum / XD minigame.",
    "achv.perfect_mystery_dungeon.title": "Perfect dungeon",
    "achv.perfect_mystery_dungeon.desc": "Complete a perfect run in the Pokémon Mystery Dungeon minigame.",
    "achv.perfect_centro_pokemon.title": "Perfect nurse",
    "achv.perfect_centro_pokemon.desc": "Complete a perfect run in the Pokémon Center Minigame category.",
    "achv.perfect_laboratorios.title": "Perfect lab coat",
    "achv.perfect_laboratorios.desc": "Complete a perfect run in the Labs Minigame category.",
    "achv.perfect_bicicletas.title": "Perfect pedal",
    "achv.perfect_bicicletas.desc": "Complete a perfect run in the Bicycles Minigame category.",
    "achv.perfect_surf.title": "Perfect wave",
    "achv.perfect_surf.desc": "Complete a perfect run in the Surf Minigame category.",
    "achv.perfect_ranger.title": "Perfect capture",
    "achv.perfect_ranger.desc": "Complete a perfect run in the Pokémon Ranger minigame.",
    "achv.perfect_title_screens.title": "Perfect cover",
    "achv.perfect_title_screens.desc": "Complete a perfect run in the Title Screens Minigame category.",
    "achv.perfect_openings_anime.title": "Perfect opening",
    "achv.perfect_openings_anime.desc": "Complete a perfect run in the Anime Openings Minigame category.",
    "achv.perfect_calle_victoria.title": "Perfect route",
    "achv.perfect_calle_victoria.desc": "Complete a perfect run in the Victory Road Minigame category.",
    "achv.hispanohablante.title": "Spanish speaker",
    "achv.hispanohablante.desc": "Play Anime Openings in both European Spanish and Latin American Spanish dubs.",
    "achv.hard_infinite_round_20.title": "Hard marathon",
    "achv.hard_infinite_round_20.desc": "Reach round 20 in Hard mode with the infinite toggle (♾️) on.",
    "achv.hard_infinite_round_50.title": "Hard ultramarathon",
    "achv.hard_infinite_round_50.desc": "Reach round 50 in Hard mode with the infinite toggle (♾️) on.",
    "achv.sonidex_1.title": "First entry",
    "achv.sonidex_1.desc": "Unlock your first Soundex entry.",
    "achv.sonidex_5.title": "First notes",
    "achv.sonidex_5.desc": "Unlock 5 Soundex entries.",
    "achv.sonidex_10.title": "Sound collector",
    "achv.sonidex_10.desc": "Unlock 10 Soundex entries.",
    "achv.sonidex_20.title": "Sharp ear",
    "achv.sonidex_20.desc": "Unlock 20 Soundex entries.",
    "achv.sonidex_50.title": "Expert music lover",
    "achv.sonidex_50.desc": "Unlock 50 Soundex entries.",
    "achv.sonidex_100.title": "Sound archive",
    "achv.sonidex_100.desc": "Unlock 100 Soundex entries.",
    "achv.sonidex_200.title": "Sound library",
    "achv.sonidex_200.desc": "Unlock 200 Soundex entries.",
    "achv.sonidex_kanto.title": "Kanto Soundex",
    "achv.sonidex_kanto.desc": "Unlock every Kanto entry.",
    "achv.sonidex_johto.title": "Johto Soundex",
    "achv.sonidex_johto.desc": "Unlock every Johto entry.",
    "achv.sonidex_hoenn.title": "Hoenn Soundex",
    "achv.sonidex_hoenn.desc": "Unlock every Hoenn entry.",
    "achv.sonidex_sinnoh.title": "Sinnoh Soundex",
    "achv.sonidex_sinnoh.desc": "Unlock every Sinnoh entry.",
    "achv.sonidex_teselia.title": "Unova Soundex",
    "achv.sonidex_teselia.desc": "Unlock every Unova entry.",
    "achv.sonidex_kalos.title": "Kalos Soundex",
    "achv.sonidex_kalos.desc": "Unlock every Kalos entry.",
    "achv.sonidex_alola.title": "Alola Soundex",
    "achv.sonidex_alola.desc": "Unlock every Alola entry.",
    "achv.story_kanto.title": "Story: Kanto",
    "achv.story_kanto.desc": "Complete Kanto in Story Mode.",
    "achv.story_johto.title": "Story: Johto",
    "achv.story_johto.desc": "Complete Johto in Story Mode.",
    "achv.story_hoenn.title": "Story: Hoenn",
    "achv.story_hoenn.desc": "Complete Hoenn in Story Mode.",
    "achv.story_sinnoh.title": "Story: Sinnoh",
    "achv.story_sinnoh.desc": "Complete Sinnoh in Story Mode.",
    "achv.story_teselia.title": "Story: Unova",
    "achv.story_teselia.desc": "Complete Unova in Story Mode.",
    "achv.story_kalos.title": "Story: Kalos",
    "achv.story_kalos.desc": "Complete Kalos in Story Mode.",
    "achv.story_complete.title": "Story Master",
    "achv.story_complete.desc": "Complete Story Mode.",
    "achv.story_complete_100.title": "Perfect story",
    "achv.story_complete_100.desc": "Complete Story Mode with 100% accuracy.",
    "achv.encounter_charizard_5.title": "Charizard sighting",
    "achv.encounter_charizard_5.desc": "Make Charizard appear 10 times.",
    "achv.encounter_charizard.title": "Flame hunter",
    "achv.encounter_charizard.desc": "Make Charizard appear 5 times.",
    "achv.encounter_slowpoke_5.title": "Slowpoke sighting",
    "achv.encounter_slowpoke_5.desc": "Make Slowpoke appear 10 times.",
    "achv.encounter_slowpoke.title": "Slowpoke's patience",
    "achv.encounter_slowpoke.desc": "Make Slowpoke appear 5 times.",
    "achv.encounter_rapidash_5.title": "Rapidash sighting",
    "achv.encounter_rapidash_5.desc": "Make Rapidash appear 10 times.",
    "achv.encounter_rapidash.title": "Rapidash speed",
    "achv.encounter_rapidash.desc": "Make Rapidash appear 5 times.",
    "achv.encounter_ditto_5.title": "Ditto sighting",
    "achv.encounter_ditto_5.desc": "Make Ditto appear 10 times.",
    "achv.encounter_ditto.title": "Ditto the impersonator",
    "achv.encounter_ditto.desc": "Make Ditto appear 5 times.",
    "achv.encounter_inkay_5.title": "Inkay sighting",
    "achv.encounter_inkay_5.desc": "Make Inkay appear 10 times.",
    "achv.encounter_inkay.title": "Inkay's spin",
    "achv.encounter_inkay.desc": "Make Inkay appear 5 times.",
    "achv.encounter_hypno_5.title": "Hypno sighting",
    "achv.encounter_hypno_5.desc": "Make Hypno appear 10 times.",
    "achv.encounter_hypno.title": "Hypno's hypnosis",
    "achv.encounter_hypno.desc": "Make Hypno appear 5 times.",
    "achv.encounter_chansey_5.title": "Chansey sighting",
    "achv.encounter_chansey_5.desc": "Make Chansey appear 10 times.",
    "achv.encounter_chansey.title": "Second chance",
    "achv.encounter_chansey.desc": "Make Chansey appear 5 times.",
    "achv.encounter_gengar_5.title": "Gengar sighting",
    "achv.encounter_gengar_5.desc": "Make Gengar appear 10 times.",
    "achv.encounter_gengar.title": "Gengar's shadow",
    "achv.encounter_gengar.desc": "Make Gengar appear 5 times.",
    "achv.encounter_pikachu_5.title": "Pikachu sighting",
    "achv.encounter_pikachu_5.desc": "Make Pikachu appear 10 times.",
    "achv.encounter_pikachu.title": "Pikachu's spark",
    "achv.encounter_pikachu.desc": "Make Pikachu appear 5 times.",
    "achv.encounter_blastoise_5.title": "Blastoise sighting",
    "achv.encounter_blastoise_5.desc": "Make Blastoise appear 10 times.",
    "achv.encounter_blastoise.title": "Rain dance",
    "achv.encounter_blastoise.desc": "Make Blastoise appear 5 times.",
    "achv.encounter_venusaur_5.title": "Venusaur sighting",
    "achv.encounter_venusaur_5.desc": "Make Venusaur appear 10 times.",
    "achv.encounter_venusaur.title": "Venusaur's scent",
    "achv.encounter_venusaur.desc": "Make Venusaur appear 5 times.",
    "achv.encounter_electrode_5.title": "Electrode sighting",
    "achv.encounter_electrode_5.desc": "Make Electrode appear 10 times.",
    "achv.encounter_electrode.title": "Countdown",
    "achv.encounter_electrode.desc": "Make Electrode appear 5 times.",
    "achv.encounter_porygon_5.title": "Porygon sighting",
    "achv.encounter_porygon_5.desc": "Make Porygon appear 10 times.",
    "achv.encounter_porygon.title": "Digital glitch",
    "achv.encounter_porygon.desc": "Make Porygon appear 5 times.",
    "achv.encounter_snorlax_5.title": "Snorlax sighting",
    "achv.encounter_snorlax_5.desc": "Make Snorlax appear 10 times.",
    "achv.encounter_snorlax.title": "Snorlax's nap",
    "achv.encounter_snorlax.desc": "Make Snorlax appear 5 times.",
    "achv.encounter_jigglypuff_5.title": "Jigglypuff sighting",
    "achv.encounter_jigglypuff_5.desc": "Make Jigglypuff appear 10 times.",
    "achv.encounter_jigglypuff.title": "Lullaby",
    "achv.encounter_jigglypuff.desc": "Make Jigglypuff appear 5 times.",
    "achv.encounter_shiny_5.title": "Shiny sighting",
    "achv.encounter_shiny_5.desc": "Find a shiny Pokémon 10 times.",
    "achv.encounter_shiny.title": "Shiny hunter",
    "achv.encounter_shiny.desc": "Find a shiny Pokémon 5 times.",
    "achv.encounter_mewtwo_5.title": "Mewtwo sighting",
    "achv.encounter_mewtwo_5.desc": "Make Mewtwo appear 10 times.",
    "achv.encounter_mewtwo.title": "Psychic clone",
    "achv.encounter_mewtwo.desc": "Make Mewtwo appear 5 times.",
    "achv.encounter_mew_5.title": "Mew sighting",
    "achv.encounter_mew_5.desc": "Make Mew appear 10 times.",
    "achv.encounter_mew.title": "Shapeshifter",
    "achv.encounter_mew.desc": "Make Mew appear 5 times.",
    "achv.poke_flute.title": "Poké Flute",
    "achv.poke_flute.desc": "Wake up a sleeping hill Pokémon by tapping it.",
    "achv.all_encounters.title": "Total sighting",
    "achv.all_encounters.desc": "Make every Pokémon Event appear at least once.",
    "achv.soldado_clicker.title": "Click soldier",
    "achv.soldado_clicker.desc": "Tap the Radio Trigal FM logo on the main menu 100 times.",

    // ── Secciones de la pantalla de Logros (ACHIEVEMENT_SECTIONS en
    // game.js): mismo criterio que los logros, solo inglés vía tData(). ──
    "achSection.progress.title": "Progress and streaks",
    "achSection.mastery.title": "Mastery and perfect runs",
    "achSection.sonidex.title": "Soundex",
    "achSection.story.title": "Story Mode",
    "achSection.encounters.title": "Pokémon Events",

    // ── Eventos Pokémon (catálogo PokeEvents en pokemon.js): mismo
    // criterio que los logros, solo inglés vía tData(). ──
    "pokeEvent.inkay.name": "Inkay",
    "pokeEvent.inkay.desc": "Inkay has appeared! Its psychic powers spin the answers 180°.",
    "pokeEvent.porygon.name": "Porygon",
    "pokeEvent.porygon.desc": "Porygon has appeared! The interface suffers a digital glitch: corrupted pixels and letters flicker like a computer error, and the song itself sounds choppy.",
    "pokeEvent.slowpoke.name": "Slowpoke",
    "pokeEvent.slowpoke.desc": "Slowpoke has appeared! This round's song plays slower.",
    "pokeEvent.gengar.name": "Gengar",
    "pokeEvent.gengar.desc": "Gengar has appeared and is hiding in the dark! Move the cursor to light up the screen, find it and tap it to be able to answer.",
    "pokeEvent.gengar.searchHint": "Gengar is hiding in the dark...<br>Light up the screen with the cursor to find it.",
    "pokeEvent.hypno.name": "Hypno",
    "pokeEvent.hypno.desc": "Hypno swings its pendulum! The screen ripples like a liquid surface and the perspective wobbles, causing intense visual dizziness.",
    "pokeEvent.chansey.name": "Chansey",
    "pokeEvent.chansey.desc": "Chansey has appeared! If you get this question wrong, it'll give you another chance.",
    "pokeEvent.rapidash.name": "Rapidash",
    "pokeEvent.rapidash.desc": "Rapidash has appeared! Its gallop speeds up this round's song.",
    "pokeEvent.shiny.name": "Shiny Caterpie",
    "pokeEvent.shiny.desc": "A Shiny Caterpie has appeared! Its colors tint the screen and multiply your points x5.",
    "pokeEvent.blastoise.name": "Blastoise",
    "pokeEvent.blastoise.desc": "Blastoise uses Rain Dance! A torrential rain falls over the battlefield.",
    "pokeEvent.charizard.name": "Charizard",
    "pokeEvent.charizard.desc": "Charizard has appeared! Its flame burns two wrong answers away.",
    "pokeEvent.pikachu.name": "Pikachu",
    "pokeEvent.pikachu.desc": "Pikachu has appeared! Its energy multiplies this round's points x3.",
    "pokeEvent.electrode.name": "Electrode",
    "pokeEvent.electrode.desc": "Electrode has appeared! It will explode at second 10 of the song and take away a life if you don't answer before then.",
    "pokeEvent.venusaur.name": "Venusaur",
    "pokeEvent.venusaur.desc": "Venusaur has appeared! Get this round right so it uses Synthesis and restores a life (max. 3).",
    "pokeEvent.ditto.name": "Ditto",
    "pokeEvent.ditto.desc": "Ditto was disguised as one of the answers and fled! One wrong option disappears.",
    "pokeEvent.jigglypuff.name": "Jigglypuff",
    "pokeEvent.jigglypuff.desc": "Jigglypuff has appeared and is about to sing its song! While it sings, the round's music plays quieter, and the correct answer will glow once it's done.",
    "pokeEvent.snorlax.name": "Snorlax",
    "pokeEvent.snorlax.desc": "Snorlax has fallen asleep on top of the answers! Tap it several times to wake it up.",
    "pokeEvent.snorlax.label": "Snorlax has fallen asleep",
    "pokeEvent.snorlax.hint": "Tap it {n} times to wake it up (0/{n})",
    "pokeEvent.snorlax.progress": "Keep tapping! ({clicks}/{n})",
    "pokeEvent.snorlax.awake": "It has woken up!",
    "pokeEvent.mewtwo.name": "Mewtwo",
    "pokeEvent.mewtwo.desc": "Mewtwo has appeared! It wraps every answer in psychic energy and conjures up two fake ones, mixed in with the real ones.",
    "pokeEvent.mew.name": "Mew",
    "pokeEvent.mew.desc": "Mew has appeared! It can transform into any other Pokémon... you choose which one.",

    // ── Modos y categorías de Minijuegos desbloqueables (MODE_UNLOCKS/
    // OTHER_UNLOCKS en game.js): mismo criterio, solo inglés vía tData().
    // reqTitle coincide con el título en inglés del logro correspondiente
    // (ver ACHIEVEMENTS/achv.*.title arriba) cuando el desbloqueo es por
    // logro, igual que ya ocurre con el texto en español en game.js. ──
    "modeUnlock.hard.name": "Hard Mode",
    "modeUnlock.hard.reqTitle": "Profile level 8",
    "modeUnlock.combat.name": "Battle Mode",
    "modeUnlock.combat.reqTitle": "Profile level 10",
    "modeUnlock.infinite.name": "Endless Challenge Mode",
    "modeUnlock.infinite.reqTitle": "Regional traveler",
    "otherUnlock.centro-pokemon.name": "Pokémon Center",
    "otherUnlock.centro-pokemon.reqTitle": "Profile level 3",
    "otherUnlock.laboratorios.name": "Labs",
    "otherUnlock.laboratorios.reqTitle": "Profile level 4",
    "otherUnlock.bicicletas.name": "Bicycles",
    "otherUnlock.bicicletas.reqTitle": "Profile level 5",
    "otherUnlock.surf.name": "Surf",
    "otherUnlock.surf.reqTitle": "Profile level 6",
    "otherUnlock.mystery-dungeon.name": "Pokémon Mystery Dungeon",
    "otherUnlock.mystery-dungeon.reqTitle": "Story: Hoenn",
    "otherUnlock.colosseum-xd.name": "Pokémon Colosseum / XD",
    "otherUnlock.colosseum-xd.reqTitle": "Music lover",
    "otherUnlock.ranger.name": "Pokémon Ranger",
    "otherUnlock.ranger.reqTitle": "Enthusiast",
    "otherUnlock.title-screens.name": "Title Screens",
    "otherUnlock.title-screens.reqTitle": "Profile level 7",
    "otherUnlock.calle-victoria.name": "Victory Road",
    "otherUnlock.calle-victoria.reqTitle": "Profile level 11",
    "otherUnlock.openings-anime.name": "Anime Openings",
    "otherUnlock.openings-anime.reqTitle": "Profile level 9",

    // Nombres de región que difieren del nombre interno (clave usada en
    // REGION_META/REGIONS de game.js y en el campo `region` de cada
    // canción). Solo hace falta listar aquí las que cambian respecto al
    // español; el resto (Kanto, Johto, Hoenn, Sinnoh, Kalos, Alola) se
    // llaman igual en los dos idiomas y regionDisplayName() ya se
    // encarga de dejarlas tal cual si no aparecen aquí.
    "region.Teselia": "Unova",

    // ── Títulos de canciones (nombre oficial en inglés de cada lugar) ──
    // Misma idea que `region.*` de arriba, pero para `song.title`
    // (ver `songDisplayName()`): clave = título original en español tal
    // y como está en el catálogo `songs` de game.js, valor = nombre
    // oficial inglés del lugar/canción. Cubre las siete regiones,
    // Combate y las categorías de Minijuegos (Laboratorios, Bicicletas/
    // Montura, Centro Pokémon, Surf, Colosseum/XD, Ranger, Pantallas de
    // Título, Calle Victoria y Openings del Anime). Quedan sin traducir
    // (se muestran en español también en inglés) los nombres propios que
    // coinciden en ambos idiomas, y todo el bloque de "Pokémon Mundo
    // Misterioso", pendientes de revisar.
    "song.Pueblo Paleta": "Pallet Town",
    "song.Ciudad Celeste": "Cerulean City",
    "song.Ciudad Verde": "Viridian City",
    "song.Ciudad Azulona": "Celadon City",
    "song.Ciudad Carmín": "Vermilion City",
    "song.Pueblo Lavanda": "Lavender Town",
    "song.Isla Canela": "Cinnabar Island",
    "song.Monte Moon": "Mt. Moon",
    "song.Meseta Añil": "Indigo Plateau",
    "song.SS Anne": "S.S. Anne",
    "song.Bosque Verde": "Viridian Forest",
    "song.Casino Rocket": "Celadon Game Corner",
    "song.Mansión Pokémon": "Pokémon Mansion",
    "song.Silph S.A.": "Silph Co.",
    "song.Torre Pokémon": "Pokémon Tower",

    // Johto
    "song.Pueblo Primavera": "New Bark Town",
    "song.Ciudad Cerezo": "Cherrygrove City",
    "song.Ciudad Malva": "Violet City",
    "song.Pueblo Azalea": "Azalea Town",
    "song.Ciudad Trigal": "Goldenrod City",
    "song.Ciudad Iris": "Ecruteak City",
    "song.Ciudad Orquídea": "Cianwood City",
    "song.Encinar": "Ilex Forest",
    "song.Torre Bellsprout": "Sprout Tower",
    "song.Torre Quemada": "Burned Tower",
    "song.Faro Ciudad Olivo": "Olivine Lighthouse",
    "song.Parque Nacional": "National Park",
    "song.Casino de Ciudad Trigal": "Goldenrod Game Corner",
    "song.Chicas Kimono": "Kimono Girls",
    "song.Ruinas Alfa": "Ruins of Alph",
    "song.Ruinas Sinjoh": "Sinjoh Ruins",
    "song.Ruta Helada": "Ice Path",

    // Hoenn
    "song.Villa Raíz": "Littleroot Town",
    "song.Pueblo Escaso": "Oldale Town",
    "song.Ciudad Férrica": "Rustboro City",
    "song.Pueblo Azuliza": "Dewford Town",
    "song.Ciudad Portual": "Slateport City",
    "song.Pueblo Verdegal": "Verdanturf Town",
    "song.Pueblo Pardal": "Fallarbor Town",
    "song.Ciudad Arborada": "Fortree City",
    "song.Ciudad Calagua": "Lilycove City",
    "song.Arrecípolis": "Sootopolis City",
    "song.Ciudad Colosalia": "Ever Grande City",
    "song.Ciudad Petalia": "Petalburg City",
    "song.Museo Oceánico": "Oceanic Museum",
    "song.Exterior del Monte Pírico": "Outside Mt. Pyre",
    "song.Frente de Batalla": "Battle Frontier",
    "song.Barco del Sr. Arenque": "Mr. Briney's Boat",
    "song.Buceo": "Underwater",
    "song.Cámara Sellada": "Sealed Chamber",
    "song.Cascada Meteoro": "Meteor Falls",
    "song.Concurso Pokémon": "Pokémon Contest",
    "song.Desierto de Hoenn": "Hoenn Desert",
    "song.Guarida del Team Aqua/Magma": "Team Aqua/Magma Hideout",
    "song.Interior del Monte Pírico": "Inside Mt. Pyre",
    "song.Nao Abandonada": "Abandoned Ship",
    "song.Pilar Celeste": "Sky Pillar",

    // Sinnoh
    "song.Ciudad Canal": "Canalave City",
    "song.Ciudad Corazón": "Hearthome City",
    "song.Ciudad Jubileo": "Jubilife City",
    "song.Ciudad Marina": "Sunyshore City",
    "song.Ciudad Pirita": "Oreburgh City",
    "song.Ciudad Puntaneva": "Snowpoint City",
    "song.Ciudad Rocavelo": "Veilstone City",
    "song.Ciudad Vetusta": "Eterna City",
    "song.Pueblo Arena": "Sandgem Town",
    "song.Pueblo Aromaflor": "Floaroma Town",
    "song.Pueblo Hojaverde": "Twinleaf Town",
    "song.Liga Pokémon Sinnoh": "Sinnoh Pokémon League",
    "song.Lago Sinnoh": "Sinnoh Lake",
    "song.Subsuelo": "Sinnoh Underground",
    "song.Bosque Vetusto": "Eterna Forest",
    "song.Pueblo Sosiego": "Solaceon Town",
    "song.Sala Final de Cintia": "Cynthia's Champion Room",
    "song.Ribera Valor": "Valor Lakefront",
    "song.Valle Eólico": "Valley Windworks",
    "song.Gran Pantano": "Great Marsh",

    // Teselia (Unova)
    "song.Ciudad Caolín": "Opelucid City",
    "song.Ciudad Engobe": "Aspertia City",
    "song.Ciudad Esmalte": "Nacrene City",
    "song.Ciudad Fayenza": "Driftveil City",
    "song.Ciudad Gres": "Striaton City",
    "song.Ciudad Hormigón": "Virbank City",
    "song.Ciudad Loza": "Mistralton City",
    "song.Ciudad Mayólica": "Nimbasa City",
    "song.Ciudad Negra": "Black City",
    "song.Ciudad Porcelana": "Castelia City",
    "song.Ciudad Teja": "Icirrus City",
    "song.Pueblo Arcilla": "Nuvema Town",
    "song.Pueblo Arenisca": "Undella Town",
    "song.Pueblo Biscuit": "Anville Town",
    "song.Pueblo Chamota": "Lentimas Town",
    "song.Pueblo Ladrillo": "Lacunosa Town",
    "song.Pueblo Ocre": "Floccesy Town",
    "song.Pueblo Terracota": "Accumula Town",
    "song.Bosque de los Perdidos": "Lostlorn Forest",
    "song.Palacio de N": "N's Castle",
    "song.Despedida de N": "N's Farewell",
    "song.Habitación de N": "N's Room",
    "song.Solar de los Sueños": "Dreamyard",
    "song.Torre Duodraco": "Dragonspiral Tower",
    "song.Un Corazón Inquebrantable": "An Unwavering Heart",

    // Kalos
    "song.Ciudad Batik": "Kiloude City",
    "song.Ciudad Fluxus": "Anistar City",
    "song.Bosque Novarte": "Santalune Forest",
    "song.Ciudad Fractal": "Snowbelle City",
    "song.Ciudad Luminalia": "Lumiose City",
    "song.Ciudad Novarte": "Santalune City",
    "song.Ciudad Relieve": "Cyllage City",
    "song.Ciudad Témpera": "Coumarine City",
    "song.Ciudad Yantra": "Shalour City",
    "song.Pueblo Acuarela": "Aquacorde Town",
    "song.Pueblo Boceto": "Vaniville Town",
    "song.Pueblo Crómlech": "Geosenge Town",
    "song.Pueblo Fresco": "Dendemille Town",
    "song.Pueblo Vánitas": "Camphrier Town",
    "song.Cueva Brillante": "Reflection Cave",
    "song.Fábrica de Poké Balls": "Poké Ball Factory",

    // Alola
    "song.Ciudad Hauoli": "Hau'oli City",
    "song.Ciudad Kantai": "Heahea City",
    "song.Ciudad Konikoni": "Konikoni City",
    "song.Ciudad Malíe": "Malie City",
    "song.Pueblo Lilii": "Iki Town",
    "song.Pueblo Ohana": "Paniola Town",
    "song.Pueblo Po": "Po Town",
    "song.Cañón de Poni": "Poni Canyon",
    "song.Paraíso Aether": "Aether Paradise",
    "song.Aldea Marina": "Seafolk Village",
    "song.Escuela de Entrenadores": "Trainers' School",
    "song.Colina Dequilate": "Ten Carat Hill",
    "song.Avenida Royale": "Royal Avenue",
    "song.Poké Resort": "Poké Pelago",

    // Combate
    "song.Salvaje Kanto": "Wild Kanto",
    "song.Salvaje Johto": "Wild Johto",
    "song.Salvaje Hoenn": "Wild Hoenn",
    "song.Salvaje Sinnoh": "Wild Sinnoh",
    "song.Salvaje Teselia": "Wild Unova",
    "song.Salvaje Kalos": "Wild Kalos",
    "song.Salvaje Alola": "Wild Alola",
    "song.Rival Johto": "Johto Rival",
    "song.Rival Hoenn": "Hoenn Rival",
    "song.Rival Sinnoh": "Sinnoh Rival",
    "song.Rival Teselia": "Unova Rival",
    "song.Rival Alola": "Alola Rival",
    "song.Amigos Kalos": "Kalos Friends",
    "song.Gimnasio Kanto": "Kanto Gym",
    "song.Gimnasio Johto": "Johto Gym",
    "song.Gimnasio Hoenn": "Hoenn Gym",
    "song.Gimnasio Sinnoh": "Sinnoh Gym",
    "song.Primeros Gimnasios Teselia": "Unova Gyms (Part 1)",
    "song.Últimos Gimnasios Teselia": "Unova Gyms (Part 2)",
    "song.Gimnasio Kalos": "Kalos Gym",
    "song.Entrenador Kanto": "Kanto Trainer",
    "song.Entrenador Johto": "Johto Trainer",
    "song.Entrenador Hoenn": "Hoenn Trainer",
    "song.Entrenador Sinnoh": "Sinnoh Trainer",
    "song.Entrenador Teselia": "Unova Trainer",
    "song.Entrenador Kalos": "Kalos Trainer",
    "song.Entrenador Alola": "Alola Trainer",
    "song.Alto Mando Hoenn": "Hoenn Elite Four",
    "song.Alto Mando Sinnoh": "Sinnoh Elite Four",
    "song.Alto Mando Teselia": "Unova Elite Four",
    "song.Alto Mando Kalos": "Kalos Elite Four",
    "song.Alto Mando Alola": "Alola Elite Four",
    "song.Pokémon Dominante": "Totem Pokémon",
    "song.Campeón Azul": "Champion Blue",
    "song.Campeona Cintia": "Champion Cynthia",
    "song.Campeona Dianta": "Champion Diantha",
    "song.Ghechis": "Ghetsis",
    "song.Guzmán": "Guzma",
    "song.Helio": "Cyrus",
    "song.Aquiles / Magno": "Archie / Maxie",
    "song.Samina": "Lusamine",
    "song.Campeón Kukui": "Champion Kukui",
    "song.Campeón Lance": "Champion Lance",
    "song.Legendario Kanto": "Kanto Legendary",
    "song.Campeón Máximo": "Champion Steven",
    "song.Lysson": "Lysandre",
    "song.Team Galaxia": "Team Galactic",
    "song.Los Regis": "The Regis",

    // Laboratorios
    "song.Laboratorio Kanto": "Kanto Lab",
    "song.Laboratorio Johto": "Johto Lab",
    "song.Laboratorio Hoenn": "Hoenn Lab",
    "song.Laboratorio Sinnoh": "Sinnoh Lab",
    "song.Laboratorio Teselia": "Unova Lab",
    "song.Laboratorio Kalos": "Kalos Lab",
    "song.Laboratorio Alola": "Alola Lab",

    // Bicicletas
    "song.Bicicleta Kanto": "Kanto Bike",
    "song.Bicicleta Johto": "Johto Bike",
    "song.Bicicleta Hoenn": "Hoenn Bike",
    "song.Bicicleta Sinnoh": "Sinnoh Bike",
    "song.Bicicleta Teselia": "Unova Bike",
    "song.Bicicleta Kalos": "Kalos Bike",
    "song.Montura Alola": "Alola Ride",

    // Centro Pokémon
    "song.Centro Pokémon Kanto": "Kanto Pokémon Center",
    "song.Centro Pokémon Johto": "Johto Pokémon Center",
    "song.Centro Pokémon Hoenn": "Hoenn Pokémon Center",
    "song.Centro Pokémon Sinnoh": "Sinnoh Pokémon Center",
    "song.Centro Pokémon Teselia": "Unova Pokémon Center",
    "song.Centro Pokémon Kalos": "Kalos Pokémon Center",
    "song.Centro Pokémon Alola": "Alola Pokémon Center",

    // Surf
    "song.Surf Kanto": "Kanto Surf",
    "song.Surf Johto": "Johto Surf",
    "song.Surf Hoenn": "Hoenn Surf",
    "song.Surf Sinnoh": "Sinnoh Surf",
    "song.Surf Teselia": "Unova Surf",
    "song.Surf Kalos": "Kalos Surf",
    "song.Montura Acuática Alola": "Alola Water Ride",

    // Pokémon Mundo Misterioso
    "song.Arboleda Chica": "Tiny Woods",
    "song.Base Equipo": "Rescue Base",
    "song.Bosque Helado": "Frosty Forest",
    "song.Bosque Niebla": "Foggy Forest",
    "song.Bosque Sombrío": "Darknight Forest",
    "song.Bosque Tétrico": "Sinister Woods",
    "song.Caverna Magma": "Magma Cavern",
    "song.Colina Oscura": "Dark Hill",
    "song.Cueva Abismo": "Chasm Cave",
    "song.Cueva Bajamar": "Beach Cave",
    "song.Cueva Cristal": "Crystal Cave",
    "song.Desierto Norte": "Northern Desert",
    "song.Dialga Primario": "Primal Dialga",
    "song.Dojo Makuhita": "Makuhita Dojo",
    "song.Fugitivos": "Fugitives",
    "song.Gran Cañón": "Great Canyon",
    "song.Manzanar": "Apple Woods",
    "song.A Través del Mar del Tiempo": "Through the Sea of Time",
    "song.Mina Onda Trueno": "Thunderwave Cave",
    "song.Monte Acero": "Mt. Steel",
    "song.Monte Árido": "Mt. Bristle",
    "song.Monte Escarcha": "Mt. Freeze",
    "song.Monte Ígneo": "Mt. Blaze",
    "song.Monte Trueno": "Mt. Thunder",
    "song.Plaza Pokémon": "Pokémon Square",
    "song.Pokégremio": "Wigglytuff's Guild",
    "song.Risco Calado": "Drenched Bluff",
    "song.Sima Silenciosa": "Silent Chasm",
    "song.Test Personalidad": "Personality Test",
    "song.Torre Del Cielo": "Sky Tower",

    // Pokémon Colosseum / XD
    "song.Básix": "The Under",
    "song.Ciudad Oasis": "Phenac City",
    "song.Guarida Equipo Cepo": "Cipher Peon Hideout",
    "song.Laboratorio Aura": "Lab (Aura)",
    "song.Laboratorio Pokémon Oscuros": "Shadow Pokémon Lab",
    "song.Monte Batalla": "Battle Mountain",
    "song.Pilar Legendario": "Relic Stone",
    "song.Pueblo Pirita": "Pyrite Town",
    "song.Puerto Ancla": "Gateon Port",
    "song.Puesto de Servicio": "Outskirt Stand",
    "song.Torre Colosal": "Colosseum",
    "song.Villa Ágata": "Agate Village",

    // Pokémon Ranger
    "song.Bosque Lira": "Lyra Forest",
    "song.Escuela Ranger": "Ranger School",
    "song.Fábrica Turnoche": "Chroma Ruins",
    "song.Otonia": "Fall City",
    "song.Puerto de Otonia": "Fall City Harbor",
    "song.Base Ranger": "Ranger Base",
    "song.Red Ranger": "Ranger Net",
    "song.Unión Ranger": "Ranger Union",
    "song.Villavera": "Ringtown",

    // Pantallas de Título
    "song.Pantalla de Título Kanto": "Kanto Title Screen",
    "song.Pantalla de Título Johto": "Johto Title Screen",
    "song.Pantalla de Título Hoenn": "Hoenn Title Screen",
    "song.Pantalla de Título Sinnoh": "Sinnoh Title Screen",
    "song.Pantalla de Título Teselia": "Unova Title Screen",
    "song.Pantalla de Título Kalos": "Kalos Title Screen",
    "song.Pantalla de Título Alola": "Alola Title Screen",

    // Calle Victoria: la pista de Alola tiene nombre propio real dentro
    // del juego (Monte Lanakila); el resto de regiones se traducen con
    // el mismo criterio que Pantallas de Título ("Victory Road" + región).
    "song.Calle Victoria Kanto": "Victory Road Kanto",
    "song.Calle Victoria Johto": "Victory Road Johto",
    "song.Calle Victoria Hoenn": "Victory Road Hoenn",
    "song.Calle Victoria Sinnoh": "Victory Road Sinnoh",
    "song.Calle Victoria Teselia": "Victory Road Unova",
    "song.Calle Victoria Kalos": "Victory Road Kalos",
    "song.Monte Lanakila": "Mount Lanakila",

    // Openings del Anime (mismas claves para las tres variantes/doblajes —
    // España/Latino/Inglés—, ya que comparten `title` en el catálogo de game.js;
    // ver la nota de songDisplayName() sobre cómo se usa esta traducción también
    // para las fichas de Sonidex, no solo mientras se juega en inglés).
    "song.Hazte con Todos": "Pokémon Theme",
    "song.Aventuras en las Islas Naranja": "Born to Be a Winner",
    "song.Los Viajes de Johto": "Pokémon Johto",
    "song.Los Campeones de la Liga de Johto": "Johto League Champions",
    "song.Master Quest": "Master Quest",
    "song.Advanced": "Advanced",
    "song.Advanced Challenge": "Advanced Challenge",
    "song.Advanced Battle": "Advanced Battle",
    "song.Battle Frontier": "Battle Frontier",
    "song.Diamante y Perla": "Diamond and Pearl",
    "song.Battle Dimension": "Battle Dimension",
    "song.Combates Galácticos": "Galactic Battles",
    "song.Los Vencedores de la Liga de Sinnoh": "Sinnoh League Victors",
    "song.Negro y Blanco": "Black and White",
    "song.Destinos Rivales": "Rival Destinies",
    "song.Aventuras en Teselia": "Adventures in Unova",
    "song.Expediciones en Kalos": "XY",
  },

  ja: {
    // ═══════════════════════════════════════════════════════════════════
    // Traducción PARCIAL: están traducidos los TÍTULOS de las
    // pantallas/menús (las claves ".title" de la navegación principal —
    // inicio, modos, minijuegos, opciones, logros, Sonidex,
    // clasificaciones, overlays de resultado...), completa, la
    // pantalla de Opciones/Ajustes (claves "options.*"), completo
    // también el catálogo de Logros (claves "achv.*", ver más abajo,
    // leídas vía tData() desde ACHIEVEMENTS en game.js), y completa
    // también la Guía de Juego (claves "guide.*", ver más abajo,
    // #screen-guide en index.html). El resto de claves (descripciones,
    // botones, estadísticas, canciones, Eventos Pokémon...) no tienen
    // aún entrada aquí, así que `t()`/`tData()` caen automáticamente al
    // español (ver el fallback a `I18N.es` dentro de `t()`) mientras se
    // va ampliando este bloque.
    // ═══════════════════════════════════════════════════════════════════
    "common.close": "閉じる",
    "common.trainerDefault": "トレーナー",
    "common.pts": "点",

    "pokeEventsInfo.btnAria": "ポケモンイベントの情報",
    "pokeEventsInfo.title": "🎲 ポケモンイベント",
    "pokeEventsInfo.subtitle": "ラウンド中、ランダムでこれらのポケモンのいずれかが特別な効果とともに現れることがあります:",

    "splash.tap": "画面をタップして始めよう",
    "splash.subtitle": "🎵 ポケクイズ・ミュージックエディション",
    // "Trigal" hace referencia a Ciudad Trigal (Goldenrod City en inglés),
    // cuyo nombre oficial en japonés es コガネシティ (Kogane City); aquí se
    // usa solo la parte del nombre de la ciudad, sin "シティ" (city), igual
    // que en español/inglés el logo dice "Radio Trigal"/"Radio Trigal" sin
    // la palabra "ciudad"/"city".
    "brand.radioTrigal": "ラジオコガネ",
    "brand.fm": "FM",
    "home.tagline": "あなたのポケモンラジオ局",

    "profileSetup.title": "ようこそ、トレーナー!",
    "profileSetup.desc": "はじめる前に、トレーナー名とプロフィール画像を選んでください。",
    "profileSetup.namePlaceholder": "トレーナー名",
    "profileSetup.avatarLabel": "プロフィール画像を選択",
    "profileSetup.confirm": "✔ プレイを始める!",
    "profileSetup.checking": "確認中…",
    "profileSetup.nameTaken": "そのトレーナー名はすでに使われています。別の名前を選んでください。",

    "profile.editNameTitle": "名前を変更",
    "profile.changeAvatarBtn": "🖼️ プロフィール画像を変更",
    "profile.levelLabel": "レベル",
    "profile.xpLabel": "レベル{next}まで {into} / {need} XP",

    "profile.stats.title": "統計",
    "profile.stats.totalPoints": "💰 合計ポイント",
    "profile.stats.achievementsUnlocked": "🏅 解除した実績数",
    "profile.stats.gamesPlayed": "🎮 プレイ回数",
    "profile.stats.correctAnswers": "✅ 正解数",
    "profile.stats.bestStreak": "🔥 最高連続正解記録",
    "profile.stats.infiniteRecord": "♾️ エンドレスチャレンジ記録",
    "profile.stats.storyRecord": "📖 ストーリーモード記録",
    "profile.stats.hardRecord": "🔴 むずかしいモード記録",
    "profile.stats.combatRecord": "⚔️ バトルモード記録",
    "profile.stats.regionRecordsTitle": "🗺️ 地方別記録",
    "profile.stats.sonidexUnlocked": "🎼 解除したサウンデックスのカード数",

    "lock.badge": "ロック中",
    "lock.levelReqBadge": "プロフィールレベル{level}で解放",
    "lock.achievementReqBadge": "実績「{title}」で解放",
    "lock.levelReqToast": "プロフィールレベル{level}に到達すると解放されます",
    "lock.achievementReqToast": "「{title}」を達成すると解放されます",
    "avatar.lockedTitle": "{name}(ロック中 — {req}が必要)",
    "avatar.lockedToast": "このアバターを解放するには{req}が必要です",
    "avatar.lockReqLevel": "プロフィールレベル{level}",
    "avatar.lockReqAchievement": "実績「{title}」",
    "avatar.lockReqUnknown": "いずれかの実績",

    "modeUnlock.hard.name": "むずかしいモード",
    "modeUnlock.hard.reqTitle": "プロフィールレベル8",
    "modeUnlock.combat.name": "バトルモード",
    "modeUnlock.combat.reqTitle": "プロフィールレベル10",
    "modeUnlock.infinite.name": "エンドレスチャレンジモード",
    "modeUnlock.infinite.reqTitle": "地方を巡る旅人",
    "otherUnlock.centro-pokemon.name": "ポケモンセンター",
    "otherUnlock.centro-pokemon.reqTitle": "プロフィールレベル3",
    "otherUnlock.laboratorios.name": "研究所",
    "otherUnlock.laboratorios.reqTitle": "プロフィールレベル4",
    "otherUnlock.bicicletas.name": "自転車",
    "otherUnlock.bicicletas.reqTitle": "プロフィールレベル5",
    "otherUnlock.surf.name": "サーフィン",
    "otherUnlock.surf.reqTitle": "プロフィールレベル6",
    "otherUnlock.mystery-dungeon.name": "ポケモン不思議のダンジョン",
    "otherUnlock.mystery-dungeon.reqTitle": "ストーリー：ホウエン",
    "otherUnlock.colosseum-xd.name": "ポケモンコロシアム / XD",
    "otherUnlock.colosseum-xd.reqTitle": "音楽好き",
    "otherUnlock.ranger.name": "ポケモンレンジャー",
    "otherUnlock.ranger.reqTitle": "熱心なファン",
    "otherUnlock.title-screens.name": "タイトル画面",
    "otherUnlock.title-screens.reqTitle": "プロフィールレベル7",
    "otherUnlock.calle-victoria.name": "チャンピオンロード",
    "otherUnlock.calle-victoria.reqTitle": "プロフィールレベル11",
    "otherUnlock.openings-anime.name": "アニメオープニング",
    "otherUnlock.openings-anime.reqTitle": "プロフィールレベル9",

    "home.play.title": "プレイ",
    "home.play.desc": "5つのモードとミニゲーム",
    "home.story.title": "ストーリーモード",
    "home.story.desc": "カントーからアローラまで、すべての地方を巡ろう",
    "home.achievements.title": "実績",
    "home.achSummaryDefault": "解除した実績を確認しよう",
    "home.sonidex.title": "サウンデックス",
    "home.sonidexSummaryDefault": "曲を10回正解して解除しよう",
    "home.leaderboard.title": "ランキング",
    "home.options.title": "設定",

    "modes.easy.title": "かんたんモード",
    "modes.easy.desc": "地方だけを当てよう（カントー、ジョウトなど…）",
    "modes.normal.title": "ノーマルモード",
    "modes.normal.desc": "まず地方を選び、その後曲を当てよう",
    "modes.hard.title": "むずかしいモード",
    "modes.hard.desc": "選択肢6つ・どのゲームの曲でも・1ラウンド10秒",
    "modes.combat.title": "バトルモード",
    "modes.combat.desc": "選択肢6つ・バトル曲を当てよう",
    "modes.infinite.title": "エンドレスチャレンジ",
    "modes.infinite.desc": "選択肢6つ・ラウンド無制限・1回のミスでゲーム終了",
    "modes.other.title": "ミニゲーム",
    "modes.other.desc": "研究所・自転車・その他のシリーズ",
    "modes.endlessToggle.title": "エンドレスモード:ラウンド無制限、1回のミスでゲーム終了",

    "region.title": "ノーマルモード",
    "region.subtitle": "地方を選んでください。曲と選択肢はその地方のものだけになります。",
    "region.Kanto": "カントー",
    "region.Johto": "ジョウト",
    "region.Hoenn": "ホウエン",
    "region.Sinnoh": "シンオウ",
    "region.Teselia": "イッシュ",
    "region.Kalos": "カロス",
    "region.Alola": "アローラ",
    "region.Música de Combate": "バトル曲",
    "region.Centros Pokémon": "ポケモンセンター",
    "region.Laboratorios": "研究所",
    "region.Bicicletas": "自転車",
    "region.Música de Surf": "サーフィン",
    "region.Pantallas de Título": "タイトル画面",
    "region.Calle Victoria": "チャンピオンロード",
    "region.Openings del Anime": "アニメオープニング",
    "region.Mundo Misterioso": "ポケモン不思議のダンジョン",
    "region.Pokémon Colosseum / XD": "ポケモンコロシアム / XD",
    "region.Pokémon Ranger": "ポケモンレンジャー",

    "other.title": "ミニゲーム",
    "other.subtitle": "カテゴリーを選んでください。選んだカテゴリーの曲だけが流れます。",
    "other.centro.title": "ポケモンセンター",
    "other.centro.desc": "ポケモンセンターの曲と選択肢のみ",
    "other.lab.title": "研究所",
    "other.lab.desc": "研究所の曲と選択肢のみ",
    "other.bike.title": "自転車",
    "other.bike.desc": "自転車の曲と選択肢のみ",
    "other.surf.title": "サーフィン",
    "other.surf.desc": "サーフィンの曲と選択肢のみ",
    "other.titlescreens.title": "タイトル画面",
    "other.titlescreens.desc": "タイトル画面の曲と選択肢のみ",
    "other.callevictoria.title": "チャンピオンロード",
    "other.callevictoria.desc": "チャンピオンロードの曲と選択肢のみ",
    "other.openinganime.title": "アニメオープニング",
    "other.openinganime.desc": "アニメオープニングの曲と選択肢のみ",
    "other.mysterydungeon.title": "ポケモン不思議のダンジョン",
    "other.mysterydungeon.desc": "ポケモン不思議のダンジョンの曲と選択肢のみ",
    "other.colosseum.title": "ポケモンコロシアム / XD",
    "other.colosseum.desc": "ポケモンコロシアム/XDの曲と選択肢のみ",
    "other.ranger.title": "ポケモンレンジャー",
    "other.ranger.desc": "ポケモンレンジャーの曲と選択肢のみ",

    "openingsLang.title": "アニメオープニング",
    "openingsLang.subtitle": "どちらの吹き替えで曲を聴きたいですか?",
    "openingsLang.spain.title": "スペイン語（スペイン）",
    "openingsLang.latino.title": "スペイン語（中南米）",

    "options.title": "設定",
    "options.subtitle": "サウンドと見た目を設定しよう。",
    "options.music.label": "音楽",
    "options.music.desc": "曲の音量",
    "options.sfx.label": "効果音",
    "options.sfx.desc": "正解・不正解の音量",
    "options.dark.label": "ダークモード",
    "options.dark.desc": "暗い場所でも見やすい暗めの画面",
    "options.toggleOn": "オン",
    "options.toggleOff": "オフ",
    "options.saveNote": "💾 この設定は自動的に保存されます。",
    "options.graphics.title": "🖼️ グラフィック設定",
    "options.bg.label": "アニメーション背景",
    "options.bg.desc": "空・雲・歩くポケモンが背景に表示されます",
    "options.particles.label": "パーティクルエフェクト",
    "options.particles.desc": "正解時や実績解除時に星や火花が出ます",
    "options.cursor.label": "🖱️ マウスカーソル",
    "options.cursor.desc": "実績で解除したスプライトに変更できます",
    "options.cursor.normalName": "ノーマル",
    "cursorStyle.poke_flute.name": "ポケモンのふえ",
    "cursorStyle.poke_ball.name": "モンスターボール",
    "cursorStyle.super_rod.name": "すごいつりざお",
    "cursorStyle.rare_candy.name": "ふしぎなアメ",
    "cursorStyle.shuckle.name": "ツボツボ",
    "cursorStyle.cosmog.name": "コスモッグ",
    "cursorStyle.togedemaru.name": "トゲデマル",
    "cursorStyle.zygarde.name": "ジガルデ",
    "cursorStyle.dedenne.name": "デデンネ",
    "cursorStyle.emolga.name": "エモンガ",
    "cursorStyle.rotom.name": "ロトム",
    "cursorStyle.pachirisu.name": "パチリス",
    "cursorStyle.plusle.name": "プラスル",
    "cursorStyle.pichu.name": "ピチュー",
    "cursorStyle.mew.name": "ミュウ",
    "cursorStyle.bulbasaur.name": "フシギダネ",
    "cursorStyle.squirtle.name": "ゼニガメ",
    "cursorStyle.charmander.name": "ヒトカゲ",
    "cursorStyle.meowth.name": "ニャース",
    "cursorStyle.charizard_y.name": "リザードンY",
    "cursorStyle.charizard_x.name": "リザードンX",
    "cursorStyle.pikachu.name": "ピカチュウ",
    "cursorStyle.spinda.name": "パッチール",
    "cursorStyle.ludicolo.name": "ルンパッパ",
    "cursorStyle.manaphy.name": "マナフィ",
    "cursorStyle.magnemite.name": "コイル",
    "cursorStyle.chansey.name": "ラッキー",
    "cursorStyle.rayquaza.name": "レックウザ",
    "cursorStyle.jigglypuff.name": "プリン",
    "cursorStyle.regigigas.name": "レジギガス",
    "cursorStyle.rayquaza_shiny.name": "色違いのレックウザ",
    "cursorStyle.meloetta.name": "メロエッタ",
    "cursorStyle.lockedTitle": "{name}(ロック中 — {req}が必要)",
    "cursorStyle.lockedToast": "{req}を達成してこのポインターを解除しよう",
    "options.language.title": "🌐 言語",
    "options.language.desc": "メニュー・実績・ポケモンイベント・ゲームガイドの言語",
    "options.language.es": "Español",
    "options.language.en": "English",
    "options.language.ja": "日本語",
    "options.guide.label": "📘 ゲームガイド",
    "options.guide.desc": "モード、得点、実績、サウンデックスの解説",

    // ─── Guía de Juego (guide.*, #screen-guide en index.html) ───────
    "guide.header.title": "📘 ゲームガイド",
    "guide.header.subtitle": "ラジオ・トリガルFMを極めるために知っておきたいことすべて。",
    "guide.modes.title": "🎮 ゲームモード",
    "guide.modes.easy.title": "🟢 かんたんモード",
    "guide.modes.easy.desc": "曲が流れるので、それがどの<b>地方</b>のものかを当てるだけです（カントー、ジョウト、ホウエンなど…）。一番シンプルなモードなので、最初に遊ぶのにぴったりです。1回のプレイは10ラウンドで、間違えても脱落せず連続正解が途切れるだけなので、常に10ラウンドすべてをプレイします。",
    "guide.modes.normal.title": "🔵 ノーマルモード",
    "guide.modes.normal.desc": "まず地方を選び、その後、同じ地方のいくつかの選択肢の中から曲の<b>正確なタイトル</b>を当てます。10ラウンドで、かんたんモードと同様、間違えてもゲームは終わらず連続正解が途切れるだけです。",
    "guide.modes.hard.title": "🔴 むずかしいモード",
    "guide.modes.hard.desc": "曲は<b>どの地方</b>のものでもあり得て、6つの選択肢から選び、1ラウンドにつき回答できるのは10秒だけです。10ラウンドで、間違えても脱落はありません。プロフィールの<b>レベル5</b>に到達すると解放されます。",
    "guide.modes.battle.title": "⚔️ バトルモード",
    "guide.modes.battle.desc": "むずかしいモードと同じですが、バトル音楽（トレーナー、ジムリーダー、ライバルなど…）のみが流れ、こちらも6つの選択肢があります。プロフィールの<b>レベル8</b>に到達すると解放されます。",
    "guide.modes.infinite.title": "♾️ エンドレスチャレンジ",
    "guide.modes.infinite.desc": "6つの選択肢で、ラウンド数の制限なく続きます。正解し続ける限りゲームは続き、最初に間違えた時点で終了します（通常、追加のハートはありません）。進めば進むほど難しく、盛り上がっていきます。実績「地方を巡る旅人」で解放されます。さらに、ストーリーモードをクリアすると、このモードでも特別なポケモンイベントが出現するようになります。",
    "guide.modes.story.title": "📖 ストーリーモード",
    "guide.modes.story.desc": "カントーからアローラまで、すべての地方を順番に旅します。各地方では10ラウンドのノーマルフェーズをプレイし、その後「強力な敵」との3ラウンドのバトルフェーズに挑みます。ここにはハート❤️があります：各地方を始めるときに3つに回復し、すべて失うとゲームが終了します。得点と連続正解数は旅の間ずっと積み重なっていきます。",
    "guide.modes.minigames.title": "🕹️ ミニゲーム",
    "guide.modes.minigames.desc": "ポケモンセンター、研究所、自転車、サーフィン、タイトル画面、チャンピオンロード、アニメオープニング、ポケモン不思議のダンジョン、ポケモンコロシアム/XD、ポケモンレンジャーなど、特定のカテゴリーに絞った短いプレイです。ほとんどは5ラウンドですが、ポケモン不思議のダンジョンのみ10ラウンドです。かんたん/ノーマル/むずかしい/バトルと同様、間違えても脱落はありません。各カテゴリーは特定の実績を達成するか、特定のプロフィールレベルに到達すると解放されます（カテゴリーによって異なり、どちらが必要かはミニゲーム画面自体で確認できます）。",
    "guide.scoring.title": "🏆 得点システム",
    "guide.scoring.speed.title": "⏱️ 回答スピード",
    "guide.scoring.speed.desc": "正解するたびに、回答の速さに応じて20〜100点の基本得点がもらえます：早く答えるほど、より多くの得点が得られます。15秒を過ぎると、正解しても保証された最低点（20点）しかもらえません。",
    "guide.scoring.streak.title": "🔥 連続正解ボーナス",
    "guide.scoring.streak.desc": "正解を連続で重ねると得点が増えます：2回連続正解目から、連続正解ごとに倍率が0.1ずつ上がります（2連続＝×1.1、3連続＝×1.2…）。11連続以上で上限の×2に達します。",
    "guide.scoring.lives.title": "❤️ ハート",
    "guide.scoring.lives.desc": "最初からハートがあるのはストーリーモードだけです（3つ、地方を移動すると回復）：1回間違えるごとに1つ減り、すべて失うとゲームが終了します。かんたん、ノーマル、むずかしい、バトル、ミニゲームにはハートがなく、間違えても連続正解が途切れるだけで、常にゲーム全体をプレイします。エンドレスチャレンジにも基本的にハートはなく、間違えると即座に終了しますが、フシギバナのイベントが発生した場合はプレイ中にハートがもらえることがあります。",
    "guide.achievements.title": "🏅 実績とアンロック要素",
    "guide.achievements.intro": "実績画面ですべての実績を確認できます。「進行と連続正解」「熟練とパーフェクトクリア」「サウンデックス」「ストーリーモード」「ポケモンイベント」の5つのグループに分かれています。⭐マークの付いた実績は、バッジに加えて新しい機能も解放します：",
    "guide.achievements.modes.title": "🎮 ゲームモード",
    "guide.achievements.modes.desc": "むずかしいモードはプロフィールレベル5、バトルモードはレベル8に到達すると解放されます。エンドレスチャレンジは実績「地方を巡る旅人」（すべての地方でプレイする）で解放されます。",
    "guide.achievements.minigames.title": "🕹️ ミニゲームのカテゴリー",
    "guide.achievements.minigames.desc": "各ミニゲームカテゴリー（ポケモンセンター、研究所、自転車、サーフィン、タイトル画面、チャンピオンロード、アニメオープニング、不思議のダンジョン、コロシアム/XD、レンジャー）は、カテゴリーごとに特定のプロフィールレベルに到達するか特定の実績を達成すると解放され、ロックされている間はボタンに🔒が表示されます。",
    "guide.achievements.events.title": "✨ ポケモンイベント",
    "guide.achievements.events.desc": "ストーリーモードをクリアする（実績「ストーリーマスター」）と、サプライズのポケモンイベント（ゲンガー、スリーパー、色違い、ポリゴン、マルマイン、カメックス、フシギバナなど…）がエンドレスチャレンジでも出現するようになります。「目撃」系の実績（例：あるポケモンを5回登場させる）は、1体ずつ、そのポケモンがメニューの背景を歩き回るようになることと、そのポケモンのプロフィールアバターを解放します。",
    "guide.achievements.sonidex.title": "🎼 サウンデックスのカード",
    "guide.achievements.sonidex.desc": "解除したカードの枚数（5、10、20、50、100、200枚）ごとの個別の実績や、各地方のサウンデックスを完成させる実績のほかに、カードを解除するために積み重ねた正解は、総正解数の実績（「音楽通」など）にもカウントされ、その一部の実績はミニゲームのカテゴリーを解放します。",
    "guide.sonidex.title": "🎼 サウンデックス",
    "guide.sonidex.intro": "サウンデックスはあなたの曲コレクションです。各曲にはロック🔒されたカードがあり、かんたんモード（地方を当てるだけで曲は当てない）以外のいずれかのモードで<b>10回</b>正解するまでロックされたままです。カードを解除すると、サウンデックスから自由にその曲を再生できるようになります。",
    "guide.sonidex.organization": "カードは地方（カントー、ジョウト、ホウエン、シンオウ、イッシュ、カロス、アローラ）、バトル音楽、そしてすべてのミニゲームカテゴリー（ポケモンセンター、研究所、自転車、サーフィン、タイトル画面、チャンピオンロード、アニメオープニング、ポケモン不思議のダンジョン、ポケモンコロシアム/XD、ポケモンレンジャー）ごとに整理されています。進捗バーと「X / Y曲解除済み」というカウンターは、ゲーム内のすべての曲を対象にしています。",
    "guide.sonidex.cities.title": "🏙️ すべての街が出てこないのはなぜ?",
    "guide.sonidex.cities.desc": "原作ゲームの中でまったく同じ曲を共有している街があるため、サウンデックスでは街ごとに1枚ずつではなく、それらをまとめて表す1枚のカードだけが表示されます。例えば、タマムシシティとクチバシティは同じ曲が流れるため、それぞれ1枚ずつではなく1枚のカードを共有しています。",
    "guide.profile.title": "🧑‍🎤 プロフィール、経験値、アバター",
    "guide.profile.xp.title": "📈 経験値とレベル",
    "guide.profile.xp.desc": "プレイ中に獲得するすべてのポイント（正解、連続正解、特別イベントなど…）は、プロフィールの経験値としても加算されます。十分な経験値がたまるとレベルアップし、レベルが上がるごとに次のレベルに必要な経験値も少しずつ増えていきます。レベルアップすることで、むずかしいモード、バトルモード、いくつかのミニゲームカテゴリー、一部のアバターが少しずつ解放されていきます（正確な条件は各解放画面で確認できます）。",
    "guide.profile.avatars.title": "🖼️ アバター",
    "guide.profile.avatars.desc": "非常に幅広いポケモンのカタログの中からアバターを選んで、プロフィールをカスタマイズできます。多くは最初から利用できますが、残りはレベルアップしたとき、ポケモンイベントのポケモンとの「目撃」実績（5回出現）を達成したとき、または特定のモードやミニゲームでパーフェクトクリアを達成したときに解放されます。プロフィールバーのアバターをタップすると、アバターやプレイヤー名を変更できます。",
    "guide.leaderboard.title": "🏆 グローバルランキング",
    "guide.leaderboard.intro": "メイン画面の🏆ボタンから、世界中のトッププレイヤーによる4つのオンラインランキングを確認できます：プロフィールレベル、エンドレスチャレンジの最高得点、ストーリーモードの最高得点、むずかしいモードの最高得点です。",
    "guide.leaderboard.update": "これら4つのカテゴリーのいずれかで自己ベストを更新するたびに、順位は自動的に更新されます。手動で送信する必要はありません。プロフィールの名前とアバターで表示されるので、ランキングで自分を分かりやすくしたい場合は、プロフィールバーからそれらをカスタマイズしてください。",

    "streaks.easyMode": "かんたんモード",
    "streaks.hardMode": "むずかしいモード",
    "streaks.combatMode": "バトルモード",
    "streaks.infiniteMode": "エンドレスチャレンジ",
    "streaks.normalByRegion": "ノーマルモード・地方別",
    "streaks.easyLabel": "🟢 かんたん",
    "streaks.hardLabel": "🔴 むずかしい",
    "streaks.combatLabel": "⚔️ バトル",
    "streaks.infiniteLabel": "♾️ チャレンジ",

    "ach.unlockedOn": "{date}に解除",

    "achievements.title": "🏅 実績",
    "achievements.streaksTitle": "🔥 最高連続正解記録",
    "achievements.progress": "{total} 個中 {n} 個解除",
    "toast.levelUpLabel": "レベルアップ",
    "toast.levelUpTitle": "レベル{n}になりました!",
    "toast.modeUnlockedLabel": "モード解放",
    "toast.minigameUnlockedLabel": "ミニゲーム解放",
    "toast.unlockedTitle": "{name}を解放しました!",
    "toast.avatarUnlockedLabel": "アバター解放",
    "toast.newAvatarTitle": "新しいアバターが使えるようになりました:{name}!",
    "toast.soundexUnlockedLabel": "カード解放",
    "toast.soundexUnlockedTitle": "サウンデックスのカードを解放しました!:{title}",
    "toast.featureUnlockedLabel": "機能解放",
    "toast.pokeEventsUnlockedTitle": "エンドレスチャレンジでポケモンイベントが解放されました!",
    "toast.newCursorTitle": "新しいカーソルが使えるようになりました:{name}!",
    "toast.newHillEncounterLabel": "新しい出会い",
    "toast.newHillEncounterTitle": "{name}が丘を歩くようになりました!",

    "feature.unlocksLabel": "が解放する機能:",
    "achFeature.closeHint": "この枠の外をタップして閉じる",
    "feature.gameMode": "ゲームモード",
    "feature.minigameCategory": "ミニゲームのカテゴリー",
    "feature.pokeEventsInInfinite": "ポケモンイベント",
    "feature.pokeEventsInInfiniteType": "エンドレスチャレンジで",
    "feature.hillPokemon": "背景のポケモン",
    "feature.hillPokemonName": "丘の{name}",
    "feature.avatarType": "プロフィールアバター",
    "feature.avatarName": "アバター:{name}",
    "feature.avatarGeneric": "アバター",

    // Nombres en japonés de los avatares del catálogo (AVATAR_CATALOG,
    // storage.js) que se pueden desbloquear mediante un logro (los que
    // aparecen en AVATAR_UNLOCKS con `achId`, game.js). Solo hace falta
    // traducir estos, no el catálogo entero: son los únicos que
    // getFeatureUnlocksForAchievement() puede mostrar en el recuadro de
    // "función desbloqueada" de la pestaña de Logros (tData busca la
    // clave "avatarName.<id>" y, si no existe, usa el nombre en inglés
    // del catálogo tal cual, por eso los avatares desbloqueables por
    // nivel no necesitan entrada aquí). Si se añade un avatar nuevo a
    // AVATAR_UNLOCKS con `achId`, añade también su nombre aquí.
    "avatarName.charizard": "リザードン",
    "avatarName.venusaur": "フシギバナ",
    "avatarName.blastoise": "カメックス",
    "avatarName.pikachu": "ピカチュウ",
    "avatarName.ditto": "メタモン",
    "avatarName.inkay": "マーイーカ",
    "avatarName.rapidash": "ギャロップ",
    "avatarName.slowpoke": "ヤドン",
    "avatarName.hypno": "スリーパー",
    "avatarName.chansey": "ラッキー",
    "avatarName.porygon": "ポリゴン",
    "avatarName.gengar": "ゲンガー",
    "avatarName.electrode": "マルマイン",
    "avatarName.snorlax": "カビゴン",
    "avatarName.jigglypuff": "プリン",
    "avatarName.caterpie": "キャタピー",
    "avatarName.mewtwo": "ミュウツー",
    "avatarName.mew": "ミュウ",
    "avatarName.espeon": "エーフィ",
    "avatarName.umbreon": "ブラッキー",
    "avatarName.dusknoir": "ヨノワール",
    "avatarName.grovyle": "ジュプトル",
    "avatarName.wigglytuff": "プクリン",
    "avatarName.lucario": "ルカリオ",
    "avatarName.dragapult": "ドラパルト",
    "avatarName.eternatus": "ムゲンダイナ",
    "avatarName.necrozma": "ネクロズマ",
    "avatarName.meltan": "メルタン",
    "avatarName.melmetal": "メルメタル",
    "avatarName.tornadus": "トルネロス",
    "avatarName.thundurus": "ボルトロス",
    "avatarName.landorus": "ランドロス",
    "avatarName.magearna": "マギアナ",
    "avatarName.terrakion": "テラキオン",
    "avatarName.virizion": "ビリジオン",
    "avatarName.cobalion": "コバルオン",
    "avatarName.volcanion": "ボルケニオン",
    "avatarName.zacian": "ザシアン",
    "avatarName.zamazenta": "ザマゼンタ",
    "avatarName.calyrex": "バドレックス",
    "avatarName.spectrier": "レイスポス",
    "avatarName.glastrier": "ブリザポス",
    "avatarName.regieleki": "レジエレキ",
    "avatarName.regidrago": "レジドラゴ",
    "avatarName.urshifu": "ウーラオス",
    "avatarName.zeraora": "ゼラオラ",
    "avatarName.lotad": "ハスボー",
    "avatarName.greninja": "ゲッコウガ",
    "avatarName.zarude": "ザルード",
    "avatarName.corviknight": "アーマーガア",
    "avatarName.cloyster": "パルシェン",
    "avatarName.butterfree": "バタフリー",
    "avatarName.pikachu-gordo": "ふとっちょピカチュウ",
    "feature.cursorName": "ポインター:{name}",
    "feature.cursorType": "ポインタースタイル",
    "toast.achievementUnlockedLabel": "実績を解除しました",
    "achSection.progress.title": "🎯 進捗と連続正解",
    "achSection.mastery.title": "🏆 熟練度とパーフェクト",
    "achSection.sonidex.title": "🎧 サウンデックス",
    "achSection.story.title": "📖 ストーリーモード",
    "achSection.encounters.title": "👻 ポケモンイベント",
    "sonidex.title": "🎼 サウンデックス",
    "sonidex.note": "🔓 同じ曲に10回正解する(かんたんモードを除く)とカードが解除されます。",
    "sonidex.progress": "{total} 曲中 {n} 曲解除",
    "sonidex.progressShort": "{total} 中 {n} 解除",
    "sonidex.changeLanguage": "オープニングの言語を変更",
    "leaderboard.title": "🏆 ランキング",
    "leaderboard.subtitle": "各カテゴリーのトップ50",
    "leaderboard.yourRecords": "🙋 あなたの記録",
    "leaderboard.level": "🧑 プレイヤーレベル",
    "leaderboard.infinite": "♾️ エンドレスチャレンジ",
    "leaderboard.story": "📖 ストーリーモード",
    "leaderboard.hard": "🔴 むずかしいモード",
    "leaderboard.combat": "⚔️ バトルモード",
    "leaderboard.regionsGroup": "🗺️ ノーマルモード、地方別",
    "leaderboard.top50": "🌍 世界のトップ50",
    "leaderboard.tab.level": "🧑 レベル",
    "leaderboard.tab.infinite": "♾️ エンドレス",
    "leaderboard.tab.story": "📖 ストーリー",
    "leaderboard.tab.hard": "🔴 むずかしい",
    "leaderboard.tab.combat": "⚔️ バトル",
    "leaderboard.tab.regions": "🗺️ 地方",
    "leaderboard.loading": "ランキングを読み込み中…",
    "leaderboard.error": "⚠️ ランキングを読み込めませんでした。あとでもう一度お試しください。",
    "leaderboard.empty": "まだ世界のランキングがありません。",
    "leaderboard.viewProfile": "{name}のプロフィールを見る",

    "publicProfile.scoresTitle": "🏆 スコア",

    "result.title": "ゲーム終了!",
    "result.scoreLabel": "スコア:",
    "result.restart": "🔄 もう一度プレイ",
    "result.exit": "🚪 終了",
    "result.attempt": "よくがんばった!",
    "result.master": "ポケモンマスター!",
    "result.great": "とてもいいね!",
    "result.good": "よくできました!",
    "result.infiniteScore": "{n}ラウンド突破 · 💰 {score}pt",
    "result.roundsScore": "{n} / {total} · 💰 {score}pt",

    "endlessInfo.title": "エンドレスモード発動!",
    "endlessInfo.body": "ラウンドは無限に続きますが、<strong>1回のミスでゲームが終了します</strong>。",
    "endlessInfo.dontShowAgain": "今後表示しない",
    "endlessInfo.ok": "👍 わかった!",

    "story.regionCompleted": "{region} クリア",
    "story.subtitleMain": "ストーリーモード",
    "story.tapStart": "画面をタップして始めよう",
    "story.enemyTitle": "強力な敵が現れた",
    "story.subtitleCombat": "バトル",
    "story.tapFight": "画面をタップして戦おう",
    "story.gameOverTitle": "ゲームオーバー",
    "story.gameOverDesc": "ライフを3つすべて失いました。カントーからやり直しましょう。",
    "story.completeTitle": "ストーリーモードクリア!",

    "confirm.leaveStoryTitle": "ストーリーモードを終了しますか?",
    "confirm.leaveStoryBody": "今ここで終了すると、このプレイの進行状況が失われます。<br>次回はまたカントーから始めることになります。",
    "confirm.keepPlaying": "🎮 プレイを続ける",
    "confirm.leaveAnyway": "🚪 終了する",

    // ─── Logros (achv.*, catálogo ACHIEVEMENTS en game.js) ──────────
    // Título y descripción de cada logro, leídos vía tData() desde
    // ui.js (ver achv.<id>.title / achv.<id>.desc más abajo).
    "achv.first_correct.title": "初めての正解",
    "achv.first_correct.desc": "初めて正解しよう。",
    "achv.streak_3.title": "ナイス連続正解",
    "achv.streak_3.desc": "3問連続で正解しよう。",
    "achv.streak_5.title": "絶好調",
    "achv.streak_5.desc": "5問連続で正解しよう。",
    "achv.streak_5_thrice.title": "常連の連続正解者",
    "achv.streak_5_thrice.desc": "5問連続正解を3回達成しよう。",
    "achv.streak_20.title": "生ける伝説",
    "achv.streak_20.desc": "20問連続で正解しよう。",
    "achv.streak_30.title": "止まらない連続正解",
    "achv.streak_30.desc": "30問連続で正解しよう。",
    "achv.streak_50.title": "伝説の連続正解",
    "achv.streak_50.desc": "50問連続で正解しよう。",
    "achv.streak_100.title": "幻の連続正解",
    "achv.streak_100.desc": "100問連続で正解しよう。",
    "achv.perfect_normal_region.title": "パーフェクトリージョン",
    "achv.perfect_normal_region.desc": "ノーマルモードのいずれかの地方をノーミスでクリアしよう。",
    "achv.perfect_easy.title": "パーフェクト・かんたん",
    "achv.perfect_easy.desc": "かんたんモードをノーミスでクリアしよう。",
    "achv.hard_correct_8.title": "チャレンジクリア",
    "achv.hard_correct_8.desc": "むずかしいモードの1プレイで8問以上正解しよう。",
    "achv.perfect_hard.title": "音楽の天才",
    "achv.perfect_hard.desc": "むずかしいモードをノーミスでクリアしよう。",
    "achv.perfect_regions_normal_5.title": "地方のスペシャリスト",
    "achv.perfect_regions_normal_5.desc": "ノーマルモードで5つの地方を正答率100%でクリアしよう。",
    "achv.correct_10.title": "良い耳",
    "achv.correct_10.desc": "10問正解しよう。",
    "achv.correct_20.title": "鍛えられた耳",
    "achv.correct_20.desc": "20問正解しよう。",
    "achv.correct_50.title": "音楽通",
    "achv.correct_50.desc": "50問正解しよう。",
    "achv.correct_100.title": "音楽好き",
    "achv.correct_100.desc": "100問正解しよう。",
    "achv.correct_250.title": "音楽マニア",
    "achv.correct_250.desc": "250問正解しよう。",
    "achv.correct_500.title": "音楽百科事典",
    "achv.correct_500.desc": "500問正解しよう。",
    "achv.games_10.title": "熱心なファン",
    "achv.games_10.desc": "10回プレイしよう。",
    "achv.games_20.title": "常連プレイヤー",
    "achv.games_20.desc": "20回プレイしよう。",
    "achv.games_30.title": "献身的なトレーナー",
    "achv.games_30.desc": "30回プレイしよう。",
    "achv.games_50.title": "ベテラン",
    "achv.games_50.desc": "50回プレイしよう。",
    "achv.games_100.title": "ラジオ・トリガルFMの伝説",
    "achv.games_100.desc": "100回プレイしよう。",
    "achv.all_modes.title": "探検家",
    "achv.all_modes.desc": "かんたん、ノーマル、むずかしいの各モードを少なくとも1回ずつクリアしよう。",
    "achv.all_regions.title": "地方を巡る旅人",
    "achv.all_regions.desc": "利用可能なすべての地方で少なくとも1回ずつプレイをクリアしよう。",
    "achv.perfect_combat.title": "バトルのエース",
    "achv.perfect_combat.desc": "バトルモードをノーミスでクリアしよう。",
    "achv.perfect_colosseum_xd.title": "パーフェクトシャドー",
    "achv.perfect_colosseum_xd.desc": "ミニゲーム「ポケモンコロシアム / XD」をノーミスでクリアしよう。",
    "achv.perfect_mystery_dungeon.title": "パーフェクトダンジョン",
    "achv.perfect_mystery_dungeon.desc": "ミニゲーム「ポケモン不思議のダンジョン」をノーミスでクリアしよう。",
    "achv.perfect_centro_pokemon.title": "パーフェクトナース",
    "achv.perfect_centro_pokemon.desc": "ミニゲーム「ポケモンセンター」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_laboratorios.title": "パーフェクト白衣",
    "achv.perfect_laboratorios.desc": "ミニゲーム「研究所」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_bicicletas.title": "パーフェクトペダル",
    "achv.perfect_bicicletas.desc": "ミニゲーム「自転車」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_surf.title": "パーフェクトウェーブ",
    "achv.perfect_surf.desc": "ミニゲーム「サーフィン」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_ranger.title": "パーフェクトキャプチャー",
    "achv.perfect_ranger.desc": "ミニゲーム「ポケモンレンジャー」をノーミスでクリアしよう。",
    "achv.perfect_title_screens.title": "パーフェクトカバー",
    "achv.perfect_title_screens.desc": "ミニゲーム「タイトル画面」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_openings_anime.title": "パーフェクトオープニング",
    "achv.perfect_openings_anime.desc": "ミニゲーム「アニメオープニング」カテゴリーをノーミスでクリアしよう。",
    "achv.perfect_calle_victoria.title": "パーフェクトロード",
    "achv.perfect_calle_victoria.desc": "ミニゲーム「チャンピオンロード」カテゴリーをノーミスでクリアしよう。",
    "achv.hispanohablante.title": "スペイン語話者",
    "achv.hispanohablante.desc": "アニメオープニングをスペイン語（スペイン）とスペイン語（中南米）の両方の吹き替えでプレイしよう。",
    "achv.hard_infinite_round_20.title": "むずかしいのマラソン",
    "achv.hard_infinite_round_20.desc": "無限モード（♾️）をオンにしたむずかしいモードでラウンド20に到達しよう。",
    "achv.hard_infinite_round_50.title": "むずかしいのウルトラマラソン",
    "achv.hard_infinite_round_50.desc": "無限モード（♾️）をオンにしたむずかしいモードでラウンド50に到達しよう。",
    "achv.sonidex_1.title": "最初の1件",
    "achv.sonidex_1.desc": "初めてサウンデックスのカードを解除しよう。",
    "achv.sonidex_5.title": "最初の音符",
    "achv.sonidex_5.desc": "サウンデックスのカードを5枚解除しよう。",
    "achv.sonidex_10.title": "サウンドコレクター",
    "achv.sonidex_10.desc": "サウンデックスのカードを10枚解除しよう。",
    "achv.sonidex_20.title": "鋭い耳",
    "achv.sonidex_20.desc": "サウンデックスのカードを20枚解除しよう。",
    "achv.sonidex_50.title": "エキスパート音楽ファン",
    "achv.sonidex_50.desc": "サウンデックスのカードを50枚解除しよう。",
    "achv.sonidex_100.title": "サウンドアーカイブ",
    "achv.sonidex_100.desc": "サウンデックスのカードを100枚解除しよう。",
    "achv.sonidex_200.title": "サウンドライブラリー",
    "achv.sonidex_200.desc": "サウンデックスのカードを200枚解除しよう。",
    "achv.sonidex_kanto.title": "カントーのサウンデックス",
    "achv.sonidex_kanto.desc": "カントー地方のカードをすべて解除しよう。",
    "achv.sonidex_johto.title": "ジョウトのサウンデックス",
    "achv.sonidex_johto.desc": "ジョウト地方のカードをすべて解除しよう。",
    "achv.sonidex_hoenn.title": "ホウエンのサウンデックス",
    "achv.sonidex_hoenn.desc": "ホウエン地方のカードをすべて解除しよう。",
    "achv.sonidex_sinnoh.title": "シンオウのサウンデックス",
    "achv.sonidex_sinnoh.desc": "シンオウ地方のカードをすべて解除しよう。",
    "achv.sonidex_teselia.title": "イッシュのサウンデックス",
    "achv.sonidex_teselia.desc": "イッシュ地方のカードをすべて解除しよう。",
    "achv.sonidex_kalos.title": "カロスのサウンデックス",
    "achv.sonidex_kalos.desc": "カロス地方のカードをすべて解除しよう。",
    "achv.sonidex_alola.title": "アローラのサウンデックス",
    "achv.sonidex_alola.desc": "アローラ地方のカードをすべて解除しよう。",
    "achv.story_kanto.title": "ストーリー：カントー",
    "achv.story_kanto.desc": "ストーリーモードでカントーをクリアしよう。",
    "achv.story_johto.title": "ストーリー：ジョウト",
    "achv.story_johto.desc": "ストーリーモードでジョウトをクリアしよう。",
    "achv.story_hoenn.title": "ストーリー：ホウエン",
    "achv.story_hoenn.desc": "ストーリーモードでホウエンをクリアしよう。",
    "achv.story_sinnoh.title": "ストーリー：シンオウ",
    "achv.story_sinnoh.desc": "ストーリーモードでシンオウをクリアしよう。",
    "achv.story_teselia.title": "ストーリー：イッシュ",
    "achv.story_teselia.desc": "ストーリーモードでイッシュをクリアしよう。",
    "achv.story_kalos.title": "ストーリー：カロス",
    "achv.story_kalos.desc": "ストーリーモードでカロスをクリアしよう。",
    "achv.story_complete.title": "ストーリーマスター",
    "achv.story_complete.desc": "ストーリーモードをクリアしよう。",
    "achv.story_complete_100.title": "パーフェクトストーリー",
    "achv.story_complete_100.desc": "ストーリーモードを正答率100%でクリアしよう。",
    "achv.encounter_charizard_5.title": "リザードン目撃",
    "achv.encounter_charizard_5.desc": "リザードンを10回登場させよう。",
    "achv.encounter_charizard.title": "炎のハンター",
    "achv.encounter_charizard.desc": "リザードンを5回登場させよう。",
    "achv.encounter_slowpoke_5.title": "ヤドン目撃",
    "achv.encounter_slowpoke_5.desc": "ヤドンを10回登場させよう。",
    "achv.encounter_slowpoke.title": "ヤドンの忍耐",
    "achv.encounter_slowpoke.desc": "ヤドンを5回登場させよう。",
    "achv.encounter_rapidash_5.title": "ギャロップ目撃",
    "achv.encounter_rapidash_5.desc": "ギャロップを10回登場させよう。",
    "achv.encounter_rapidash.title": "ギャロップの速さ",
    "achv.encounter_rapidash.desc": "ギャロップを5回登場させよう。",
    "achv.encounter_ditto_5.title": "メタモン目撃",
    "achv.encounter_ditto_5.desc": "メタモンを10回登場させよう。",
    "achv.encounter_ditto.title": "ものまね上手のメタモン",
    "achv.encounter_ditto.desc": "メタモンを5回登場させよう。",
    "achv.encounter_inkay_5.title": "マーイーカ目撃",
    "achv.encounter_inkay_5.desc": "マーイーカを10回登場させよう。",
    "achv.encounter_inkay.title": "マーイーカの回転",
    "achv.encounter_inkay.desc": "マーイーカを5回登場させよう。",
    "achv.encounter_hypno_5.title": "スリーパー目撃",
    "achv.encounter_hypno_5.desc": "スリーパーを10回登場させよう。",
    "achv.encounter_hypno.title": "スリーパーの催眠術",
    "achv.encounter_hypno.desc": "スリーパーを5回登場させよう。",
    "achv.encounter_chansey_5.title": "ラッキー目撃",
    "achv.encounter_chansey_5.desc": "ラッキーを10回登場させよう。",
    "achv.encounter_chansey.title": "セカンドチャンス",
    "achv.encounter_chansey.desc": "ラッキーを5回登場させよう。",
    "achv.encounter_gengar_5.title": "ゲンガー目撃",
    "achv.encounter_gengar_5.desc": "ゲンガーを10回登場させよう。",
    "achv.encounter_gengar.title": "ゲンガーの影",
    "achv.encounter_gengar.desc": "ゲンガーを5回登場させよう。",
    "achv.encounter_pikachu_5.title": "ピカチュウ目撃",
    "achv.encounter_pikachu_5.desc": "ピカチュウを10回登場させよう。",
    "achv.encounter_pikachu.title": "ピカチュウの火花",
    "achv.encounter_pikachu.desc": "ピカチュウを5回登場させよう。",
    "achv.encounter_blastoise_5.title": "カメックス目撃",
    "achv.encounter_blastoise_5.desc": "カメックスを10回登場させよう。",
    "achv.encounter_blastoise.title": "あまごい",
    "achv.encounter_blastoise.desc": "カメックスを5回登場させよう。",
    "achv.encounter_venusaur_5.title": "フシギバナ目撃",
    "achv.encounter_venusaur_5.desc": "フシギバナを10回登場させよう。",
    "achv.encounter_venusaur.title": "フシギバナの香り",
    "achv.encounter_venusaur.desc": "フシギバナを5回登場させよう。",
    "achv.encounter_electrode_5.title": "マルマイン目撃",
    "achv.encounter_electrode_5.desc": "マルマインを10回登場させよう。",
    "achv.encounter_electrode.title": "カウントダウン",
    "achv.encounter_electrode.desc": "マルマインを5回登場させよう。",
    "achv.encounter_porygon_5.title": "ポリゴン目撃",
    "achv.encounter_porygon_5.desc": "ポリゴンを10回登場させよう。",
    "achv.encounter_porygon.title": "デジタルの不具合",
    "achv.encounter_porygon.desc": "ポリゴンを5回登場させよう。",
    "achv.encounter_snorlax_5.title": "カビゴン目撃",
    "achv.encounter_snorlax_5.desc": "カビゴンを10回登場させよう。",
    "achv.encounter_snorlax.title": "カビゴンの昼寝",
    "achv.encounter_snorlax.desc": "カビゴンを5回登場させよう。",
    "achv.encounter_jigglypuff_5.title": "プリン目撃",
    "achv.encounter_jigglypuff_5.desc": "プリンを10回登場させよう。",
    "achv.encounter_jigglypuff.title": "子守唄",
    "achv.encounter_jigglypuff.desc": "プリンを5回登場させよう。",
    "achv.encounter_shiny_5.title": "色違い目撃",
    "achv.encounter_shiny_5.desc": "色違いのポケモンを10回見つけよう。",
    "achv.encounter_shiny.title": "色違いハンター",
    "achv.encounter_shiny.desc": "色違いのポケモンを5回見つけよう。",
    "achv.encounter_mewtwo_5.title": "ミュウツー目撃",
    "achv.encounter_mewtwo_5.desc": "ミュウツーを10回登場させよう。",
    "achv.encounter_mewtwo.title": "エスパーのクローン",
    "achv.encounter_mewtwo.desc": "ミュウツーを5回登場させよう。",
    "achv.encounter_mew_5.title": "ミュウ目撃",
    "achv.encounter_mew_5.desc": "ミュウを10回登場させよう。",
    "achv.encounter_mew.title": "変身自在",
    "achv.encounter_mew.desc": "ミュウを5回登場させよう。",
    "achv.poke_flute.title": "ポケモンのふえ",
    "achv.poke_flute.desc": "丘で眠っているポケモンをタップして起こそう。",
    "achv.all_encounters.title": "コンプリート目撃",
    "achv.all_encounters.desc": "すべてのポケモンイベントを少なくとも1回ずつ登場させよう。",
    "achv.soldado_clicker.title": "クリック戦士",
    "achv.soldado_clicker.desc": "メインメニューのラジオ・トリガルFMのロゴを100回タップしよう。",

    // ─── Títulos de canciones (nombre oficial en japonés) ───────────
    // Misma idea que la sección equivalente del bloque `en` (ver
    // "song.*" ahí): clave = título original en español tal y como
    // está en el catálogo `songs` de game.js, valor = nombre oficial
    // japonés del lugar/canción. De momento cubre Kanto, Johto, Hoenn,
    // Sinnoh, Teselia, Kalos y Alola (las siete regiones principales),
    // Combate, Pokémon Mundo Misterioso, Pokémon Colosseum/XD,
    // Pokémon Ranger y los Minijuegos de Laboratorios, Bicicletas/
    // Montura, Centro Pokémon, Surf, Pantallas de Título y Calle
    // Victoria; el resto de categorías de Minijuegos quedan pendientes
    // y siguen cayendo al español (o, si existe, al inglés) vía el
    // fallback de `tData()`.
    "song.Pueblo Paleta": "マサラタウン",
    "song.Ciudad Celeste": "ハナダシティ",
    "song.Ciudad Verde": "トキワシティ",
    "song.Ciudad Azulona": "タマムシシティ",
    "song.Ciudad Carmín": "クチバシティ",
    "song.Pueblo Lavanda": "シオンタウン",
    "song.Isla Canela": "グレン島",
    "song.Monte Moon": "つきのふもと",
    "song.Meseta Añil": "セキエイ高原",
    "song.SS Anne": "サントアンヌ号",
    "song.Bosque Verde": "トキワの森",
    "song.Casino Rocket": "ゲームコーナー",
    "song.Mansión Pokémon": "ポケモンやしき",
    "song.Silph S.A.": "シルフカンパニー",
    "song.Torre Pokémon": "ポケモンタワー",

    // Johto
    "song.Pueblo Primavera": "ワカバタウン",
    "song.Ciudad Cerezo": "ヨシノシティ",
    "song.Ciudad Malva": "キキョウシティ",
    "song.Pueblo Azalea": "ヒワダタウン",
    "song.Ciudad Trigal": "コガネシティ",
    "song.Ciudad Iris": "エンジュシティ",
    "song.Ciudad Orquídea": "タンバシティ",
    "song.Encinar": "ウバメの森",
    "song.Torre Bellsprout": "マダツボミのとう",
    "song.Torre Quemada": "やけたとう",
    "song.Faro Ciudad Olivo": "アサギのとうだい",
    "song.Parque Nacional": "しぜんこうえん",
    "song.Casino de Ciudad Trigal": "ゲームコーナー",
    "song.Chicas Kimono": "まいこはん",
    "song.Ruinas Sinjoh": "シントいせき",
    "song.Ruta Helada": "こおりのぬけみち",

    // Hoenn
    "song.Villa Raíz": "ミシロタウン",
    "song.Pueblo Escaso": "コトキタウン",
    "song.Ciudad Férrica": "カナズミシティ",
    "song.Pueblo Azuliza": "ムロタウン",
    "song.Ciudad Portual": "カイナシティ",
    "song.Pueblo Verdegal": "シダケタウン",
    "song.Pueblo Pardal": "ハジツゲタウン",
    "song.Ciudad Arborada": "ヒワマキシティ",
    "song.Ciudad Calagua": "ミナモシティ",
    "song.Arrecípolis": "キナギタウン",
    "song.Ciudad Colosalia": "ルネシティ",
    "song.Ciudad Petalia": "トウカシティ",
    "song.Museo Oceánico": "海の科学博物館",
    "song.Exterior del Monte Pírico": "おくりび山",
    "song.Frente de Batalla": "バトルフロンティア",
    "song.Barco del Sr. Arenque": "タイドリップ号",
    "song.Buceo": "ダイビング",
    "song.Cámara Sellada": "おふれの石室",
    "song.Cascada Meteoro": "りゅうせいのたき",
    "song.Concurso Pokémon": "ポケモンコンテスト",
    "song.Desierto de Hoenn": "ホウエンの砂漠",
    "song.Guarida del Team Aqua/Magma": "アクア団/マグマ団アジト",
    "song.Interior del Monte Pírico": "おくりび山",
    "song.Nao Abandonada": "すてられ船",
    "song.Pilar Celeste": "空の柱",

    // Sinnoh
    "song.Ciudad Canal": "ミオシティ",
    "song.Ciudad Corazón": "ヨスガシティ",
    "song.Ciudad Jubileo": "コトブキシティ",
    "song.Ciudad Marina": "ナギサシティ",
    "song.Ciudad Pirita": "クロガネシティ",
    "song.Ciudad Puntaneva": "キッサキシティ",
    "song.Ciudad Rocavelo": "トバリシティ",
    "song.Ciudad Vetusta": "ハクタイシティ",
    "song.Pueblo Arena": "マサゴタウン",
    "song.Pueblo Aromaflor": "ソノオタウン",
    "song.Pueblo Hojaverde": "フタバタウン",
    "song.Liga Pokémon Sinnoh": "ポケモンリーグ",
    "song.Lago Sinnoh": "シンオウの湖",
    "song.Subsuelo": "ちかつうろ",
    "song.Bosque Vetusto": "ハクタイの森",
    "song.Pueblo Sosiego": "ズイタウン",
    "song.Sala Final de Cintia": "シロナの最後の部屋",
    "song.Ribera Valor": "リッシ湖のほとり",
    "song.Valle Eólico": "たにまのはつでんしょ",
    "song.Gran Pantano": "ノモセ大湿原",

    // Teselia (Unova)
    "song.Ciudad Caolín": "ソウリュウシティ",
    "song.Ciudad Engobe": "ヒオウギシティ",
    "song.Ciudad Esmalte": "シッポウシティ",
    "song.Ciudad Fayenza": "ホドモエシティ",
    "song.Ciudad Gres": "サンヨウシティ",
    "song.Ciudad Hormigón": "タチワキシティ",
    "song.Ciudad Loza": "フキヨセシティ",
    "song.Ciudad Mayólica": "ライモンシティ",
    "song.Ciudad Negra": "ブラックシティ",
    "song.Ciudad Porcelana": "ヒウンシティ",
    "song.Ciudad Teja": "セッカシティ",
    "song.Pueblo Arcilla": "カノコタウン",
    "song.Pueblo Arenisca": "サザナミタウン",
    "song.Pueblo Biscuit": "カナワタウン",
    "song.Pueblo Chamota": "ヤマジタウン",
    "song.Pueblo Ladrillo": "カゴメタウン",
    "song.Pueblo Ocre": "サンギタウン",
    "song.Pueblo Terracota": "カラクサタウン",
    "song.Bosque de los Perdidos": "迷いの森",
    "song.Palacio de N": "Ｎの城",
    "song.Despedida de N": "Ｎとの別れ",
    "song.Habitación de N": "Ｎの部屋",
    "song.Solar de los Sueños": "ゆめのあとち",
    "song.Torre Duodraco": "リュウラセンの塔",
    "song.Un Corazón Inquebrantable": "折れない心",

    // Kalos
    "song.Ciudad Batik": "キナンシティ",
    "song.Ciudad Fluxus": "ヒャッコクシティ",
    "song.Bosque Novarte": "ハクダンの森",
    "song.Ciudad Fractal": "エイセツシティ",
    "song.Ciudad Luminalia": "ミアレシティ",
    "song.Ciudad Novarte": "ハクダンシティ",
    "song.Ciudad Relieve": "ショウヨウシティ",
    "song.Ciudad Témpera": "ヒヨクシティ",
    "song.Ciudad Yantra": "シャラシティ",
    "song.Pueblo Acuarela": "メイスイタウン",
    "song.Pueblo Boceto": "アサメタウン",
    "song.Pueblo Crómlech": "セキタイタウン",
    "song.Pueblo Fresco": "フウジョタウン",
    "song.Pueblo Vánitas": "コボクタウン",
    "song.Cueva Brillante": "輝きの洞窟",
    "song.Fábrica de Poké Balls": "ボール工場",

    // Alola
    "song.Ciudad Hauoli": "ハウオリシティ",
    "song.Ciudad Kantai": "カンタイシティ",
    "song.Ciudad Konikoni": "コニコシティ",
    "song.Ciudad Malíe": "マリエシティ",
    "song.Pueblo Lilii": "リリィタウン",
    "song.Pueblo Ohana": "オハナタウン",
    "song.Pueblo Po": "ポータウン",
    "song.Cañón de Poni": "ポニの大峡谷",
    "song.Paraíso Aether": "エーテルパラダイス",
    "song.Aldea Marina": "海の民の村",
    "song.Escuela de Entrenadores": "トレーナーズスクール",
    "song.Colina Dequilate": "テンカラットヒル",
    "song.Avenida Royale": "ロイヤルアベニュー",
    "song.Poké Resort": "ポケリゾート",

    // Combate
    "song.Salvaje Kanto": "カントー野生ポケモン",
    "song.Salvaje Johto": "ジョウト野生ポケモン",
    "song.Salvaje Hoenn": "ホウエン野生ポケモン",
    "song.Salvaje Sinnoh": "シンオウ野生ポケモン",
    "song.Salvaje Teselia": "イッシュ野生ポケモン",
    "song.Salvaje Kalos": "カロス野生ポケモン",
    "song.Salvaje Alola": "アローラ野生ポケモン",
    "song.Rival Johto": "ジョウトライバル",
    "song.Rival Hoenn": "ホウエンライバル",
    "song.Rival Sinnoh": "シンオウライバル",
    "song.Rival Teselia": "イッシュライバル",
    "song.Rival Alola": "アローラライバル",
    "song.Amigos Kalos": "カロスともだち",
    "song.Gimnasio Kanto": "カントージムリーダー",
    "song.Gimnasio Johto": "ジョウトジムリーダー",
    "song.Gimnasio Hoenn": "ホウエンジムリーダー",
    "song.Gimnasio Sinnoh": "シンオウジムリーダー",
    "song.Primeros Gimnasios Teselia": "イッシュ序盤のジムリーダー",
    "song.Últimos Gimnasios Teselia": "イッシュ終盤のジムリーダー",
    "song.Gimnasio Kalos": "カロスジムリーダー",
    "song.Entrenador Kanto": "カントートレーナー",
    "song.Entrenador Johto": "ジョウトトレーナー",
    "song.Entrenador Hoenn": "ホウエントレーナー",
    "song.Entrenador Sinnoh": "シンオウトレーナー",
    "song.Entrenador Teselia": "イッシュトレーナー",
    "song.Entrenador Kalos": "カロストレーナー",
    "song.Entrenador Alola": "アローラトレーナー",
    "song.Alto Mando Hoenn": "ホウエン四天王",
    "song.Alto Mando Sinnoh": "シンオウ四天王",
    "song.Alto Mando Teselia": "イッシュ四天王",
    "song.Alto Mando Kalos": "カロス四天王",
    "song.Alto Mando Alola": "アローラ四天王",
    "song.Pokémon Dominante": "ヌシポケモン",
    "song.Campeón Azul": "チャンピオングリーン",
    "song.Campeona Cintia": "チャンピオンシロナ",
    "song.Campeona Dianta": "チャンピオンカルネ",
    "song.Ghechis": "ゲーチス",
    "song.Guzmán": "グズマ",
    "song.Helio": "アカギ",
    "song.Aquiles / Magno": "アオギリ／マツブサ",
    "song.Samina": "ルザミーネ",
    "song.Campeón Kukui": "チャンピオンククイ",
    "song.Campeón Lance": "チャンピオンワタル",
    "song.Legendario Kanto": "カントー伝説ポケモン",
    "song.Campeón Máximo": "チャンピオンダイゴ",
    "song.Lysson": "フラダリ",
    "song.Team Galaxia": "ギンガ団",
    "song.Regis": "レジ系",
    "song.Kahuna": "しまキング・しまクイーン",
    "song.Arceus": "アルセウス",
    "song.Deoxys": "デオキシス",
    "song.Giratina": "ギラティナ",
    "song.Kyogre / Groudon": "カイオーガ／グラードン",
    "song.Uxie/Mesprit/Azelf": "ユクシー／エムリット／アグノム",
    "song.N": "N",
    "song.Palkia / Dialga": "パルキア／ディアルガ",
    "song.Reshiram / Zekrom": "レシラム／ゼクロム",
    "song.Solgaleo / Lunala": "ソルガレオ／ルナアーラ",
    "song.Tapus": "カプ",
    "song.Team Rocket": "ロケット団",
    "song.Team Aqua / Magma": "アクア団/マグマ団",
    "song.Team Plasma": "プラズマ団",
    "song.Team Flare": "フレア団",
    "song.Team Skull": "スカル団",
    "song.Team Aether": "エーテル財団",
    "song.Xerneas / Yveltal": "ゼルネアス／イベルタル",
    "song.Lugia": "ルギア",
    "song.Ho-Oh": "ホウオウ",
    "song.Entei": "エンテイ",
    "song.Raikou": "ライコウ",
    "song.Suicune": "スイクン",

    // Pokémon Mundo Misterioso
    "song.Arboleda Chica": "小さな森",
    "song.Base Equipo": "救助隊基地",
    "song.Bosque Helado": "氷雪の霊峰",
    "song.Bosque Niebla": "霧の森",
    "song.Bosque Sombrío": "怪しい森",
    "song.Bosque Tétrico": "呪われた森",
    "song.Caverna Magma": "マグマの地底",
    "song.Colina Oscura": "闇の洞窟",
    "song.Cueva Abismo": "奈落の洞窟",
    "song.Cueva Bajamar": "潮の洞窟",
    "song.Cueva Cristal": "水晶の洞窟",
    "song.Desierto Norte": "北の砂漠",
    "song.Dialga Primario": "原始のダンジョン",
    "song.Dojo Makuhita": "マクノシタ道場",
    "song.Fugitivos": "逃避行",
    "song.Gran Cañón": "大いなる峡谷",
    "song.Manzanar": "リンゴの森",
    "song.A Través del Mar del Tiempo": "時の海を越えて",
    "song.Mina Onda Trueno": "電磁波の洞窟",
    "song.Monte Acero": "ハガネ山",
    "song.Monte Árido": "荒れた大地",
    "song.Monte Escarcha": "樹氷の森",
    "song.Monte Ígneo": "炎の山",
    "song.Monte Trueno": "雷鳴の山",
    "song.Plaza Pokémon": "ポケモン広場",
    "song.Pokégremio": "プクリンのギルド",
    "song.Risco Calado": "沈黙の谷",
    "song.Sima Silenciosa": "沈黙の谷",
    "song.Test Personalidad": "性格診断",
    "song.Torre Del Cielo": "天空の塔",

    // Pokémon Colosseum / XD
    "song.Básix": "アンダー",
    "song.Ciudad Oasis": "フェナスシティ",
    "song.Guarida Equipo Cepo": "スナッチ団アジト",
    "song.Laboratorio Aura": "ラボラトリー",
    "song.Laboratorio Pokémon Oscuros": "シャドーＰＯＫÉモン研究所",
    "song.Monte Batalla": "バトル山",
    "song.Pilar Legendario": "聖なる遺跡",
    "song.Pueblo Pirita": "パイラタウン",
    "song.Puerto Ancla": "アイオポート",
    "song.Puesto de Servicio": "町外れのスタンド",
    "song.Torre Colosal": "コロシアム",
    "song.Villa Ágata": "アゲトビレッジ",

    // Pokémon Ranger
    "song.Bosque Lira": "ライラの森",
    "song.Escuela Ranger": "レンジャースクール",
    "song.Fábrica Turnoche": "ヨナベこうじょう",
    "song.Otonia": "フォルシティ",
    "song.Puerto de Otonia": "フォルシティ港",
    "song.Base Ranger": "レンジャーベース",
    "song.Red Ranger": "レンジャーネット",
    "song.Unión Ranger": "レンジャーユニオン",
    "song.Villavera": "リングタウン",

    // Laboratorios
    "song.Laboratorio Kanto": "カントーの研究所",
    "song.Laboratorio Johto": "ジョウトの研究所",
    "song.Laboratorio Hoenn": "ホウエンの研究所",
    "song.Laboratorio Sinnoh": "シンオウの研究所",
    "song.Laboratorio Teselia": "イッシュの研究所",
    "song.Laboratorio Kalos": "カロスの研究所",
    "song.Laboratorio Alola": "アローラの研究所",

    // Bicicletas
    "song.Bicicleta Kanto": "カントーの自転車",
    "song.Bicicleta Johto": "ジョウトの自転車",
    "song.Bicicleta Hoenn": "ホウエンの自転車",
    "song.Bicicleta Sinnoh": "シンオウの自転車",
    "song.Bicicleta Teselia": "イッシュの自転車",
    "song.Bicicleta Kalos": "カロスの自転車",
    "song.Montura Alola": "アローラのポケモンライド",

    // Centro Pokémon
    "song.Centro Pokémon Kanto": "カントーのポケモンセンター",
    "song.Centro Pokémon Johto": "ジョウトのポケモンセンター",
    "song.Centro Pokémon Hoenn": "ホウエンのポケモンセンター",
    "song.Centro Pokémon Sinnoh": "シンオウのポケモンセンター",
    "song.Centro Pokémon Teselia": "イッシュのポケモンセンター",
    "song.Centro Pokémon Kalos": "カロスのポケモンセンター",
    "song.Centro Pokémon Alola": "アローラのポケモンセンター",

    // Surf
    "song.Surf Kanto": "カントーのなみのり",
    "song.Surf Johto": "ジョウトのなみのり",
    "song.Surf Hoenn": "ホウエンのなみのり",
    "song.Surf Sinnoh": "シンオウのなみのり",
    "song.Surf Teselia": "イッシュのなみのり",
    "song.Surf Kalos": "カロスのなみのり",
    "song.Montura Acuática Alola": "アローラの水上ライド",

    // Pantallas de Título
    "song.Pantalla de Título Kanto": "カントーのタイトル画面",
    "song.Pantalla de Título Johto": "ジョウトのタイトル画面",
    "song.Pantalla de Título Hoenn": "ホウエンのタイトル画面",
    "song.Pantalla de Título Sinnoh": "シンオウのタイトル画面",
    "song.Pantalla de Título Teselia": "イッシュのタイトル画面",
    "song.Pantalla de Título Kalos": "カロスのタイトル画面",
    "song.Pantalla de Título Alola": "アローラのタイトル画面",

    // Calle Victoria: la pista de Alola tiene nombre propio real dentro
    // del juego (Monte Lanakila); el resto de regiones se traducen con
    // el nombre oficial japonés de la Calle Victoria ("チャンピオンロード")
    // combinado con la región, mismo criterio que Pantallas de Título.
    "song.Calle Victoria Kanto": "カントーのチャンピオンロード",
    "song.Calle Victoria Johto": "ジョウトのチャンピオンロード",
    "song.Calle Victoria Hoenn": "ホウエンのチャンピオンロード",
    "song.Calle Victoria Sinnoh": "シンオウのチャンピオンロード",
    "song.Calle Victoria Teselia": "イッシュのチャンピオンロード",
    "song.Calle Victoria Kalos": "カロスのチャンピオンロード",
    "song.Monte Lanakila": "ラナキラマウンテン",

    // ─── Eventos Pokémon (pokemon.js) ───────────────────────────────
    // Claves leídas vía tData() desde pokemon.js/ui.js/game.js — nombre
    // y descripción de la carta de aparición de cada evento del Modo
    // Historia, más los textos propios de los eventos de Gengar y
    // Snorlax (pista de búsqueda / etiqueta y progreso de toques).
    "pokeEvent.tag": "✨ ポケモンイベント",

    "pokeEvent.inkay.name": "マーイーカ",
    "pokeEvent.inkay.desc": "マーイーカが現れた!サイコパワーで選択肢が180度回転する。",
    "pokeEvent.porygon.name": "ポリゴン",
    "pokeEvent.porygon.desc": "ポリゴンが現れた!画面にデジタルの不具合が発生し、ノイズや文字化けが明滅する。曲も途切れ途切れに聞こえる。",
    "pokeEvent.slowpoke.name": "ヤドン",
    "pokeEvent.slowpoke.desc": "ヤドンが現れた!このラウンドの曲がゆっくり再生される。",
    "pokeEvent.gengar.name": "ゲンガー",
    "pokeEvent.gengar.desc": "ゲンガーが現れて暗闇に隠れた!カーソルを動かして画面を照らし、見つけてタップすると回答できる。",
    "pokeEvent.gengar.searchHint": "ゲンガーは暗闇に隠れている…<br>カーソルで画面を照らして見つけよう。",
    "pokeEvent.hypno.name": "スリーパー",
    "pokeEvent.hypno.desc": "スリーパーが振り子を揺らす!画面が液体のように波打ち、視点が揺れて強いめまいを引き起こす。",
    "pokeEvent.chansey.name": "ラッキー",
    "pokeEvent.chansey.desc": "ラッキーが現れた!この問題を間違えても、もう一度チャンスがもらえる。",
    "pokeEvent.rapidash.name": "ギャロップ",
    "pokeEvent.rapidash.desc": "ギャロップが現れた!その疾走でこのラウンドの曲が速くなる。",
    "pokeEvent.shiny.name": "色違いのキャタピー",
    "pokeEvent.shiny.desc": "色違いのキャタピーが現れた!その色が画面を染め、得点が5倍になる。",
    "pokeEvent.blastoise.name": "カメックス",
    "pokeEvent.blastoise.desc": "カメックスがあまごいを使った!激しい雨がフィールドに降り注ぐ。",
    "pokeEvent.charizard.name": "リザードン",
    "pokeEvent.charizard.desc": "リザードンが現れた!その炎が誤った選択肢を2つ焼き尽くす。",
    "pokeEvent.pikachu.name": "ピカチュウ",
    "pokeEvent.pikachu.desc": "ピカチュウが現れた!そのパワーでこのラウンドの得点が3倍になる。",
    "pokeEvent.electrode.name": "マルマイン",
    "pokeEvent.electrode.desc": "マルマインが現れた!曲の10秒地点で爆発する。それまでに答えないとライフが1つ減ってしまう。",
    "pokeEvent.venusaur.name": "フシギバナ",
    "pokeEvent.venusaur.desc": "フシギバナが現れた!このラウンドに正解すると、こうごうせいを使ってライフが1つ回復する(最大3)。",
    "pokeEvent.ditto.name": "メタモン",
    "pokeEvent.ditto.desc": "選択肢の一つに化けていたメタモンが逃げ出した!誤った選択肢が1つ消える。",
    "pokeEvent.jigglypuff.name": "プリン",
    "pokeEvent.jigglypuff.desc": "プリンが現れて歌い始めた!歌っている間はラウンドの曲の音量が下がり、歌い終わると正解の選択肢が光る。",
    "pokeEvent.snorlax.name": "カビゴン",
    "pokeEvent.snorlax.desc": "カビゴンが選択肢の上で眠ってしまった!何度もタップして起こそう。",
    "pokeEvent.snorlax.label": "カビゴンが眠ってしまった",
    "pokeEvent.snorlax.hint": "{n}回タップして起こそう (0/{n})",
    "pokeEvent.snorlax.progress": "その調子でタップ!({clicks}/{n})",
    "pokeEvent.snorlax.awake": "目を覚ました!",
    "pokeEvent.mewtwo.name": "ミュウツー",
    "pokeEvent.mewtwo.desc": "ミュウツーが現れた!すべての選択肢をサイコパワーで包み込み、本物に紛れて偽の選択肢を2つ作り出す。",
    "pokeEvent.mew.name": "ミュウ",
    "pokeEvent.mew.desc": "ミュウが現れた!ほかのどのポケモンにも変身できる…どれに変身するかはあなた次第。",

    // ═══════════════════════════════════════════════════════════════════
    // 🎮 Cabecera de la ronda y pantalla del quiz (claves "header.*" y
    // "quiz.*"): añadidas para que la partida en sí también se juegue en
    // japonés, no solo los menús.
    // ═══════════════════════════════════════════════════════════════════
    "header.back": "戻る",
    "header.points": "ポイント",
    "header.round": "ラウンド",
    "header.streak": "連続正解",

    "quiz.loading": "読み込み中…",
    "quiz.hint": "👁️ ビジュアルヒント",
    "quiz.hintUsed": "👁️ ヒント使用済み",
    "quiz.hintCost": "-50%ポイント",
    "quiz.next": "▶ 次のラウンドへ",
    "quiz.modeEasy": "モード:かんたん・地方を当てよう",
    "quiz.modeHard": "モード:むずかしい・選択肢6つ・1ラウンド10秒",
    "quiz.hardTimerUnit": "秒",
    "quiz.modeInfinite": "モード:エンドレスチャレンジ・選択肢6つ・1回のミスでゲーム終了",
    "quiz.modeCombat": "モード:バトル・選択肢6つ",
    "quiz.modeNormalRegion": "モード:ノーマル・地方:{region}",
    "quiz.modeOther": "モード:ミニゲーム・{game}",
    "quiz.modeStoryEnemy": "📖 ストーリーモード・⚔️ 強力な敵",
    "quiz.modeStoryRegion": "📖 ストーリーモード・地方:{region}",
    "quiz.questionRegion": "🌍 この曲はどの地方のもの?",
    "quiz.questionSong": "🎶 流れているのはどの曲?",
    "quiz.playing": "▶ 再生中…",
    "quiz.tapToPlay": "▶ 画面をタップして再生",
    "quiz.chanseySecondChance": "💗 ラッキーがもう一度チャンスをくれた!",
    "quiz.hardTimeUp": "⏱️ 時間切れ!",
  },
};

// ═══════════════════════════════════════════════
//  🔤 TRADUCIR Y APLICAR
// ═══════════════════════════════════════════════

/**
 * Devuelve el texto traducido para `key` en el idioma actual
 * (`settings.language`), sustituyendo placeholders `{nombre}` por los
 * valores de `vars`. Si la clave no existe, devuelve la propia clave
 * (para que un texto sin traducir sea visible/fácil de detectar en vez
 * de romper la interfaz).
 * @param {string} key
 * @param {Object<string, string|number>} [vars]
 * @returns {string}
 */
function t(key, vars) {
  const dict = I18N[settings.language] || I18N.es;
  let str = dict[key];
  if (str === undefined) str = I18N.es[key];
  if (str === undefined) return key;
  if (vars) {
    for (const k in vars) str = str.split(`{${k}}`).join(String(vars[k]));
  }
  return str;
}

/**
 * Como t(), pero para textos que viven como VALOR POR DEFECTO en otro
 * fichero en vez de en el propio diccionario `I18N` — el catálogo de
 * logros (`ACHIEVEMENTS` en game.js), el catálogo de Eventos Pokémon
 * (`pokemon.js`) o los desbloqueables (`MODE_UNLOCKS`/`OTHER_UNLOCKS`
 * en game.js). Esos catálogos solo tienen su texto en español (para no
 * duplicarlo, siguiendo la Regla nº2 de CLAUDE.md); esta función deja
 * ese español como está y solo lo sustituye si existe una traducción
 * para `key` en el idioma actual. A diferencia de `t()`, NUNCA devuelve
 * la propia `key`: si no hay traducción, devuelve `defaultText` tal
 * cual.
 * @param {string} key
 * @param {string} defaultText  el texto en español ya presente en el
 *   catálogo de origen, usado tal cual si no hay traducción.
 * @param {Object<string, string|number>} [vars]
 * @returns {string}
 */
function tData(key, defaultText, vars) {
  const dict = I18N[settings.language];
  let str = dict ? dict[key] : undefined;
  if (str === undefined) return defaultText;
  if (vars) {
    for (const k in vars) str = str.split(`{${k}}`).join(String(vars[k]));
  }
  return str;
}

/**
 * Como tData(), pero específica para nombres de región (la clave
 * interna usada en REGIONS/REGION_META de game.js y en el campo
 * `region` de cada canción — p. ej. "Teselia"). Devuelve la propia
 * región tal cual si el idioma actual no tiene una entrada
 * "region.<región>" (caso de la mayoría, que se llaman igual en
 * español e inglés). Los ficheros que solo NECESITAN la clave interna
 * (comparar `song.region`, guardar en localStorage, comprobar logros...)
 * deben seguir usando esa clave sin pasar por aquí; esta función es
 * solo para el texto que ve el jugador.
 * @param {string} region  clave interna de la región (p. ej. "Kanto",
 *   "Teselia"...).
 * @returns {string}
 */
function regionDisplayName(region) {
  return tData(`region.${region}`, region);
}

/**
 * Devuelve el título traducido de una canción para mostrarlo al
 * jugador (si existe traducción para su título original en español,
 * clave `song.<título original>`, ver más abajo). Si no hay traducción
 * para ese título, se devuelve tal cual (igual que `regionDisplayName`).
 * Importante: el título ORIGINAL (`song.title`, el que viene del
 * catálogo `songs` de game.js) sigue siendo la clave interna que se usa
 * para comparar/identificar canciones (p. ej. al generar las opciones
 * de respuesta o comprobar el acierto) — esa comparación NUNCA pasa por
 * aquí, solo el texto que ve el jugador en pantalla.
 */
function songDisplayName(song) {
  return tData(`song.${song.title}`, song.title);
}

/**
 * Aplica el idioma actual a todo el marcado estático de index.html
 * marcado con `data-i18n` (texto), `data-i18n-placeholder` (atributo
 * placeholder), `data-i18n-title` (atributo title) o
 * `data-i18n-aria` (atributo aria-label). Se llama al arrancar la app y
 * cada vez que el jugador cambia de idioma en Opciones.
 */
function applyTranslations() {
  document.querySelectorAll("[data-i18n]").forEach(el => {
    el.textContent = t(el.getAttribute("data-i18n"));
  });
  document.querySelectorAll("[data-i18n-html]").forEach(el => {
    el.innerHTML = t(el.getAttribute("data-i18n-html"));
  });
  document.querySelectorAll("[data-i18n-placeholder]").forEach(el => {
    el.placeholder = t(el.getAttribute("data-i18n-placeholder"));
  });
  document.querySelectorAll("[data-i18n-title]").forEach(el => {
    el.title = t(el.getAttribute("data-i18n-title"));
  });
  document.querySelectorAll("[data-i18n-aria]").forEach(el => {
    el.setAttribute("aria-label", t(el.getAttribute("data-i18n-aria")));
  });
  document.documentElement.lang = settings.language;
}

/**
 * Cambia el idioma de los menús, lo persiste (storage.js) y refresca
 * tanto el marcado estático (`applyTranslations`) como las pantallas ya
 * renderizadas dinámicamente por ui.js que dependan del idioma (las que
 * estén cargadas en ese momento).
 * @param {"es"|"en"|"ja"} lang
 */
function setLanguage(lang) {
  if (lang !== "es" && lang !== "en" && lang !== "ja") return;
  settings.language = lang;
  saveSettings();
  applyTranslations();
  if (typeof refreshLanguageDependentUI === "function") refreshLanguageDependentUI();
}
